...
def get_squeeze_signal(df, idx):
    ...
    if (history_h.max() - close) < 50:
        return "BUY_SQUEEZE"
    if (close - history_l.min()) < 50:
        return "SELL_SQUEEZE"
    return "NEUTRAL"
...
def run_potential_audit():
            ...
            opps["BUY"].append(
                {
                    ...
                    "mae": mae,
                    "atr": row["atr"],
                    "type": "SQUEEZE" if sqz == "BUY_SQUEEZE" else "BASE",
                }
            )
            ...
            opps["SELL"].append(
                {
                    ...
                    "mae": mae,
                    "atr": row["atr"],
                    "type": "SQUEEZE" if sqz == "SELL_SQUEEZE" else "BASE",
                }
            )
    ...
    for side in ["BUY", "SELL"]:
        ...
        if total > 0:
            ...
            print(
                f"  - Taxa de Acerto (>70pts): {len(assertive)} ({(len(assertive) / total) * 100:.1f}%)"
            )
            if not assertive.empty:
                print(f"  - MFE Médio (Ganhos Max): {assertive['mfe'].mean():.1f} pts")
            print(
                f"  - Sinais Falsos (MAE >150): {len(full_loss)} ({(len(full_loss) / total) * 100:.1f}%)"
            )
            ...
            # Dinheiro na mesa
            if not assertive.empty:
                potencial_reais = len(assertive) * (
                    70 * 0.20 * 3
                )  # 70pts * 20c * 3Lot = R$42/trade
                print(
                    f"  => Ganho Potencial (Conservador @ 70pts/3Lot): R$ {potencial_reais:.2f}"
                )
