...
async def run():
    ...
    locked_params["use_ai_core"] = (
        False  # [ANTIVIBE-CODING] Conforme v22_locked_params.json
    )
    ...
    print(
        "  TIMEFRAME: M1  |  ESTRATÉGIA: SOTA V28 (flux=0.95, cooldown=7min, Filtro-H ativo)"
    )
    ...
    for dia in DIAS_ALVO:
        ...
        # --- Análise de Trades ---
        trades_df = (
            pd.DataFrame(result["trades"]) if result["trades"] else pd.DataFrame()
        )
        compras = (
            trades_df[trades_df["side"] == "buy"]
        ...
        print(f"\n  {'RESULTADO GERAL':}")
        print(
            f"  ├── PNL Total          : {fmt_reais(pnl_total):>15}  {pct(pnl_total, INITIAL_BAL):>10}"
        )
        ...
        if not compras.empty:
            ...
            print(
                f"  Qtd: {n_compras}  |  Ganhos: {wins_c}  |  Perdas: {loss_c}  |  PNL: {fmt_reais(pnl_compras)}"
            )
            print(
                f"  {'Entrada':<22} {'Saída':<22} {'Preço E':>9} {'Preço S':>9} {'Pts':>7} {'PNL':>12}  {'Motivo':<12}"
            )
            print(f"  {'-' * 95}")
            for _, t in compras.iterrows():
                ...
                pts = t.get("pnl_pts", 0)
                print(
                    f"  {sinal} {str(t['entry_time'])[:19]:<21} {str(t['exit_time'])[:19]:<21} "
        ...
        if not vendas.empty:
            ...
            print(
                f"  Qtd: {n_vendas}  |  Ganhos: {wins_v}  |  Perdas: {loss_v}  |  PNL: {fmt_reais(pnl_vendas)}"
            )
            print(
                f"  {'Entrada':<22} {'Saída':<22} {'Preço E':>9} {'Preço S':>9} {'Pts':>7} {'PNL':>12}  {'Motivo':<12}"
            )
        ...
        print(
            f"  Bloqueados pela IA        : {missed_ai}  (confiança abaixo do limiar)"
        )
        print(f"  Bloqueados pelo filtro flux: {missed_flux}")
        print(
            f"  Bloqueados Filtro-H (tend.): {missed_bias}  [MELHORIA H V28 — veto de direção]"
        )
        if tiers:
    ...
    print(
        f"  ├── PNL Consolidado       : {fmt_reais(total_pnl_geral):>15}  {pct(total_pnl_geral, INITIAL_BAL):>10}"
    )
    print(f"  ├── PNL de Compras        : {fmt_reais(total_pnl_compras):>15}")
    print(f"  ├── PNL de Vendas         : {fmt_reais(total_pnl_vendas):>15}")
    print(
        f"  ├── Capital Final Estimado: {fmt_reais(INITIAL_BAL + total_pnl_geral):>15}"
    )
    print("  │")
    print(f"  ├── Dias Positivos        : {dias_positivos}/{len(resultados_dias)}")
    print(
        f"  ├── Melhor Dia            : {melhor_dia['data']} → {fmt_reais(melhor_dia['pnl_total'])}"
    )
    print(
        f"  └── Pior Dia              : {pior_dia['data']} → {fmt_reais(pior_dia['pnl_total'])}"
    )
    ...
    print(
        f"  ├── Bloqueados pela IA             : {total_missed_ai}  (confiança < limiar 70%)"
    )
    print(f"  ├── Bloqueados pelo filtro de fluxo: {total_missed_flux}")
    print(
        f"  ├── Bloqueados Filtro-H (tend.)    : {summary['missed_bias'].sum()}  [MELHORIA H V28]"
    )
    pct_aproveitamento = (total_trades_geral / total_v22 * 100) if total_v22 > 0 else 0
    print(
        f"  └── Taxa de aproveitamento V22     : {pct_aproveitamento:.1f}%  ({total_trades_geral}/{total_v22})"
    )
    ...
    # 4. Poucos trades?
    media_trades_dia = (
        total_trades_geral / len(resultados_dias) if resultados_dias else 0
    )
    ...
    for r in resultados_dias:
        ...
        print(
            f"  {sinal} {r['data']:<11} {r['n_trades']:>7} {r['n_compras']:>5} {r['n_vendas']:>6} "
    ...
    print(f"  {'─' * 80}")
    print(
        f"  {'TOTAL':<12} {total_trades_geral:>7} {total_compras:>5} {total_vendas:>6} "
