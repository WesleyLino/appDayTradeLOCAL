...
async def run_optimization():
        ...
        for p_signal in pyramid_signals:
            for p_max in pyramid_max_vols:
                ...
                try:
                    ...
                    results.append(res)
                    logger.info(f"[{current_run}/{total_runs}] P:{p_profit} S:{p_signal} V:{p_max} | PnL: R${res['pnl']:.2f} | DD: {res['max_dd_pct']:.2f}%")
    ...
    if champions:
        ...
        for i, c in enumerate(champions[:3]):
            logger.info(f"#{i+1}: Profits={c['pyramid_profit_threshold']} | Signal={c['pyramid_signal_threshold']} | MaxVol={c['pyramid_max_volume']} | PnL={c['pnl']:.2f} | DD={c['max_dd_pct']:.2f}%")
    else:
        logger.warning("⚠️ Nenhuma configuração respeitou o limite de 15% de Drawdown. Sugerindo revisão de risco.")
