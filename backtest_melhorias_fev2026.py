                ...
                trades.append(
                    {
                        ...
                        "entry": entry,
                        "exit": close
                        if exit_reason != "SL/Trail"
                        else (
                            max(sl, trail_sl) if side == "buy" else min(sl, trail_sl)
                        ),
...
def main():
    ...
    print(
        f"  Trailing SELL   : {TRAILING_TRIGGER_SELL}/{TRAILING_LOCK_SELL}/{TRAILING_STEP_SELL} pts (vs 70/50/20 antes)"
    )
    print(
        f"  Scaling Out     : Parcial em +{PARTIAL_PROFIT_PTS} pts ({PARTIAL_LOTS} contrato)"
    )
    print(
        f"  Quarter-Kelly   : {int(KELLY_REDUCTION * 100)}% do lote após 2 perdas → restaura em {KELLY_WIN_COUNT} acertos"
    )
    print()
    ...
    for day in DIAS:
        ...
        if rates is None or len(rates) < 20:
            print(f"\n{'─' * 80}")
            print(
                f"  DIA {day.strftime('%d/%02m/%Y')} — Sem dados disponíveis. Pulando."
            )
            continue
        ...
        print(
            f"  ├── PNL Total       : R$ {day_pnl:+.2f}  ({day_pnl / INITIAL_CAPITAL * 100:+.2f}%)"
        )
        ...
        print("  ── COMPRAS (BUY) ——————————————————————")
        print(
            f"  Qtd: {len(buy_tr)} | Ganhos: {buy_wins} | Perdas: {len(buy_tr) - buy_wins} | PNL: R$ {resultado['buy_pnl']:+.2f}"
        )
        for t in buy_tr:
            icon = "✅" if t["won"] else "❌"
            p_tag = " [PARCIAL]" if t.get("had_partial") else ""
            print(
                f"  {icon} {t['entry_time'].strftime('%H:%M')}→{t['exit_time'].strftime('%H:%M')}  "
        ...
        print("  ── VENDAS (SELL) ——————————————————————")
        print(
            f"  Qtd: {len(sell_tr)} | Ganhos: {sel_wins} | Perdas: {len(sell_tr) - sel_wins} | PNL: R$ {resultado['sell_pnl']:+.2f}"
        )
        for t in sell_tr:
            icon = "✅" if t["won"] else "❌"
            p_tag = " [PARCIAL]" if t.get("had_partial") else ""
        ...
        print(
            f"  Bloq. Cooldown : {day_shad['cooldown']}  ← Quarter-Kelly não bloqueia, só reduz lote"
        )
    ...
    print(
        f"  ├── PNL Consolidado       : R$ {total_pnl:+.2f}  ({total_pnl / INITIAL_CAPITAL * 100:+.2f}%)"
    )
    ...
    if melhor_dia:
        print(
            f"  ├── Melhor Dia            : {melhor_dia['day'].strftime('%d/%m/%Y')} → R$ {melhor_dia['pnl']:+.2f}"
        )
    if pior_dia:
        print(
            f"  └── Pior Dia              : {pior_dia['day'].strftime('%d/%m/%Y')} → R$ {pior_dia['pnl']:+.2f}"
        )
    print()
    print("  OPERAÇÕES")
    ...
    print(
        f"  └── Taxa Aproveit. V22    : {(total_trades / total_v22 * 100):.1f}%  (vs 36.6% no baseline)"
        if total_v22
        else ""
    )
    print()
