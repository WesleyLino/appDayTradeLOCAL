...
async def run_deep_search():
    ...
    print(f"📊 Testando {len(thresholds) * len(multipliers)} combinações em {len(dates)} dias.")
    print("="*60)
    ...
    for t in thresholds:
        for m in multipliers:
            ...
            for d in dates:
                res = await audit_day_params(bridge, d, symbol, {'threshold': t, 'multiplier': m}, data_cache)
                if res:
    ...
    print("\n\n" + "="*80)
    print(f"{'RANK':<5} | {'THR':<5} | {'MULT':<5} | {'PNL TOTAL':<12} | {'AVG/DAY':<10} | {'TRADES':<6} | {'MAX DD':<8}")
    print("-" * 80)
    
    for i, r in enumerate(sorted_results[:10]):
        rank_icon = "🏆" if i == 0 else "🥈" if i == 1 else "🥉" if i == 2 else f" {i+1} "
        print(f"{rank_icon:<5} | {r['threshold']:<5.2f} | {r['multiplier']:<5.2f} | R$ {r['total_pnl']:<9.2f} | R$ {r['avg_pnl_day']:<7.2f} | {r['total_trades']:<6} | {r['max_dd']:>6.2f}%")
