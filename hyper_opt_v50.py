...
async def hyper_optimize():
    ...
    with ProcessPoolExecutor(max_workers=workers) as executor:
        future_to_params = {
            executor.submit(run_single_backtest, c, data_files): c for c in combinations
        }
    ...
    for i, champ in enumerate(champions):
        p = champ["params"]
        print(
            f"#{i + 1} | PnL Total: R$ {champ['pnl']:.2f} | Trades: {champ['trades']} | WR: {champ['wr']:.1f}% | DD: {champ['max_dd']:.1%}"
        )
        print(
            f"     Conf: {p['confidence_threshold']} | VWAP: {p['vwap_dist_threshold']} | V-Spike: {p['vol_spike_mult']} | RSI: {p['rsi_buy_level']}/{p['rsi_sell_level']}\n"
        )
