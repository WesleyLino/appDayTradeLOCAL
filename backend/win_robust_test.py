...
def perform_wfa(symbol="WIN$", n_folds=3):
    ...
    logging.info(
        f"🧬 Starting WFA for {symbol} | {n_folds} Folds | Total: {total_len} rows"
    )
    ...
    for fold in range(n_folds):
        ...
        logging.info(
            f"📂 Processing Fold {fold + 1}/{n_folds} (Rows {start_idx} to {end_idx})"
        )
        ...
        with ProcessPoolExecutor(max_workers=os.cpu_count()) as executor:
            futures = {
                executor.submit(run_single_backtest, params, temp_fold_file): params
    ...
    for fold_id, results in enumerate(final_results):
        for res in results:
            ...
            stability_metrics[params_key]["max_dd"] = max(
                stability_metrics[params_key]["max_dd"], res["max_dd"]
            )
    ...
    champions = sorted(champions, key=lambda x: x["avg_profit_factor"], reverse=True)[
        :5
    ]
    ...
    for i, champ in enumerate(champions):
        print(
            f"#{i + 1} | Avg PF: {champ['avg_profit_factor']:.2f} | Total PnL: R${champ['total_net_profit']:.2f} | Max DD: {champ['max_drawdown']:.2%}"
        )
