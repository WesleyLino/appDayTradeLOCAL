...
async def run_multi_day_audit():
    ...
    for data_str in datas_alvo:
        ...
        bt = BacktestPro(symbol="WIN$", timeframe="M1")
        ...
        report = (
            await bt.run()
        )  # O BacktestPro v22 já puxa os candles mais recentes se n_candles for passado

        # Filtramos os trades do dia
        trades = [
            t
            for t in report.get("trades", [])
            if pd.to_datetime(t["exit_time"]).date() == data_obj.date()
        ]
