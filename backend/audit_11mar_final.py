...
async def run_audit():
    try:
        ...
        if not trades.empty:
            ...
            print(f"📈 TRADE COMPRA:  Qtde: {len(buys):<3} | PnL: R$ {buys['pnl_fin'].sum():.2f}")
            print(f"📉 TRADE VENDA:   Qtde: {len(sells):<3} | PnL: R$ {sells['pnl_fin'].sum():.2f}")
            print(f"❌ PREJUÍZO ACUM: R$ {trades[trades['pnl_fin'] < 0]['pnl_fin'].sum():.2f}")
            print(f"✅ ACERTOS: {len(trades[trades['pnl_fin'] > 0])} | 📉 ERROS: {len(trades[trades['pnl_fin'] <= 0])}")
        else:
            print("Nenhum trade executado sob as condições atuais (Filtros Conservadores).")
        ...
        print("1. Ajuste de Bias H1: O mercado teve rali forte às 14h. Relaxar RSI em bias de alta.")
        print("2. Calibragem Volatilidade: O ATR inicial estava alto. Reduzir redução de lote em breakouts.")
