...
async def test_execution_logic():
    ...
    class MockBridge:
        ...
        def get_current_symbol(self, base):
            return "WINJ24"
    ...
    print(
        f"TP: {params['tp']} (Esperado ~{current_price - 10 + 300} - mult 1.5 tendencia)"
    )
    ...
    print(
        f"TP: {params_lat['tp']} (Esperado ~{current_price + 10 - 80} - mult 0.8 lateral)"
    )
    ...
    print(
        f"SL: {params_momentum['sl']} (Distância: {abs(params_momentum['sl'] - current_price):.0f} pts | Esperado: ~{expected_sl_dist})"
    )
    print(
        f"TP: {params_momentum['tp']} (Distância: {abs(params_momentum['tp'] - current_price):.0f} pts | Esperado: ~{expected_tp_dist})"
    )
    ...
    if abs(params_momentum["sl"] - current_price) >= (expected_sl_dist - 20):
        print(
            "✅ SUCESSO: SL Institucional expandido corretamente (com tolerância Anti-Violinada)."
        )
    else:
