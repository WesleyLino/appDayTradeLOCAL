    ...
    try:
        ...
        with open(artifact_path, "w", encoding="utf-8") as f:
            ...
            if not trades.empty:
                ...
                f.write(
                    f"- **Operações COMPRADAS:** {len(buys)} trades | PnL: R$ {buys['pnl_fin'].sum():.2f}\n"
                )
                f.write(
                    f"- **Operações VENDIDAS:** {len(sells)} trades | PnL: R$ {sells['pnl_fin'].sum():.2f}\n"
                )
                f.write(f"- **Prejuízo (Loss):** R$ {prejuizo:.2f}\n")
                f.write(
                    f"- **Taxa de Acerto:** {(len(trades[trades['pnl_fin'] > 0]) / len(trades) * 100):.1f}%\n\n"
                )
            else:
            ...
            f.write(
                f"- **Alertas V22 (Sinais Candidatos):** {shadow.get('v22_candidates', 0)}\n"
            )
            f.write(
                f"- **Vetos por Insegurança IA:** {shadow.get('filtered_by_ai', 0)}\n"
            )
            f.write(
                f"- **Vetos por Gerenciamento de Risco:** {shadow.get('filtered_by_risk', 0)}\n\n"
            )
            ...
            f.write(
                "1. **Momentum Bias**: Ativar rali dinâmico quando ADX > 35 no M5.\n"
            )
            f.write(
                "2. **Proteção de Capital**: Trailing stop mais curto em dias de alta volatilidade.\n"
            )
