...
async def run_macro_audit():
    ...
    for target_date in dates_to_test:
        ...
        if date_str == "10/03/2026" and len(day_trades) > 0:
            ...
            for idx, trade in day_trades.iterrows():
                print(
                    f"   [{trade.get('entry_time')}] {trade.get('side').upper()} @ {trade.get('entry_price')} | SL: {trade.get('sl')} | PnL: {trade.get('pnl_fin')} | Modo: {trade.get('execution_mode', 'IA')}"
                )
    ...
    total_vetos = tester.shadow_signals.get(
        "filtered_by_ai", 0
    ) + tester.shadow_signals.get("filtered_by_flux", 0)
    ...
    report += (
        f"- O potencial de **COMPRA** gerou R$ {pnl_buys:.2f} ({total_buys} trades).\n"
    )
    report += (
        f"- O potencial de **VENDA** gerou R$ {pnl_sells:.2f} ({total_sells} trades).\n"
    )
    report += (
