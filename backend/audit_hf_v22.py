...
# Força UTF-8 para evitar crash com emojis no Windows
...
async def run_expert_bi_audit():
    ...
    v36_expert_config = {
        "confidence_buy_threshold": 0.57,
        "confidence_sell_threshold": 0.43,
    ...
    print("[SYNC] Sincronizando dados históricos WIN$ (15 dias)...")
    ...
    if data_full is None or data_full.empty:
        print("[ERRO] Erro ao carregar dados.")
        return
    ...
    print(
        f"{'Data':<10} | {'PnL':<8} | {'Compra':<8} | {'Venda':<8} | {'Loss':<8} | {'Opt. Perd.'}"
    )
    print("-" * 70)
    ...
    for dia_str in dias:
        ...
        lucro_compra = sum(
            t["pnl_fin"] for t in day_trades if t["side"] == "buy" and t["pnl_fin"] > 0
        )
        lucro_venda = sum(
            t["pnl_fin"] for t in day_trades if t["side"] == "sell" and t["pnl_fin"] > 0
        )
        # Prejuízo (Gross Loss)
        ...
        print(
            f"{dia_str:<10} | {pnl_dia:>8.2f} | {lucro_compra:>8.2f} | {lucro_venda:>8.2f} | {abs(loss_dia):>8.2f} | {opt_perdidas:^10}"
        )
    ...
    print(
        f"   Gross Profit (C+V): R$ {consolidado['total_compra'] + consolidado['total_venda']:.2f}"
    )
    ...
    with open("backend/audit_hf_v22_results.json", "w") as f:
        json.dump(
            {"consolidado": consolidado, "detalhes": resultados_diarios}, f, indent=4
        )
