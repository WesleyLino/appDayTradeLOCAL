...
async def run_v12_audit():
    ...
    params['use_wdo_filter'] = True
    # Nota: V12 usa Smart Sizing dinâmico (1, 2, 3 contratos) baseado em score Bayesiano.
    params.pop('force_lots', None) 
    
    initial_capital_per_day = 3000.0
    
    print("\n" + "="*95)
    print("🕵️ AUDITORIA SOTA V12 (SNIPER PRIME): ÚLTIMOS 30 DIAS INDIVIDUAIS")
    print(f"Ativo: WIN$ | Capital por Pregão: R$ {initial_capital_per_day:.2f} | Local: MT5 Histórico")
    print("="*95 + "\n")
    ...
    target_days = unique_days[-31:-1] # Pegamos os 30 dias anteriores ao último (para evitar dados incompletos de hoje)
    ...
    for day in target_days:
        ...
        start_idx_with_padding = max(0, day_start_idx - 150) # Mais padding para VWAP e ATR
        ...
        # Injetar dados
        bt.df = data_chunk
        async def mock_load_data(): return data_chunk
        ...
        print(f"{day.strftime('%d/%m/%Y'):<12} | R$ {pnl_buy:>7.2f} | R$ {pnl_sell:>7.2f} | R$ {total_pnl:>7.2f} | {day_count:>6} | {day_wr:>4.0f}% | {missed:>6}")
    ...
    output_path = "backend/audit_v12_raw_data.json"
    with open(output_path, "w", encoding="utf-8") as f:
        # Converter dates para str
        for r in results: r['date'] = str(r['date'])
        json.dump(results, f, indent=4)

if __name__ == "__main__":
    asyncio.run(run_v12_audit())
