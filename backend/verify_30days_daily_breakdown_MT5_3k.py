...
async def run_daily_breakdown():
    ...
    for i, day in enumerate(target_days):
        ...
        if report is None:
            print(f"{day.strftime('%d/%m/%Y'):<12} | {'ERRO':>10} | {'-':>6} | {'-':>5} | {'-':>6} | ⚠️ FALHA")
            continue
    ...
    print(f"MELHOR DIA:................. {best_day['date'].strftime('%d/%m/%Y')} (R$ {best_day['pnl']:.2f})")
    print(f"PIOR DIA:................... {worst_day['date'].strftime('%d/%m/%Y')} (R$ {worst_day['pnl']:.2f})")
    print("="*85 + "\n")
