...
def collect_today():
    ...
    for symbol in symbols:
        ...
        if df_ticks is not None and not df_ticks.empty and 'flags' in df_ticks.columns:
            ...
            df = df.merge(agg[['time', 'cvd', 'ofi', 'volume_ratio']], on='time', how='left')
        else:
            body = df['close'] - df['open']
            df['cvd'] = df['real_volume'] * body.apply(lambda x: 1 if x > 0 else -1 if x < 0 else 0)
            df['ofi'] = body / (df['high'] - df['low'] + 1e-8)
            df['volume_ratio'] = df['real_volume'] / (df['real_volume'].rolling(20).mean() + 1e-8)
        ...
        logging.info(f"✅ Dados de hoje consolidados para {symbol}: {len(df_labeled)} linhas.")
