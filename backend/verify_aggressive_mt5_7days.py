...
async def run_aggressive_mt5_7day_test():
    print("📈 INICIANDO TESTE AGRESSIVO (7 DIAS) - DADOS REAIS MT5")
    ...
    # 2,800 candles M1 ~= 7 dias operacionais (cerca de 400 candles por dia na janela Sniper 10h-12h, 
    # ou 540-600 candles se considerar o dia todo. BacktestPro usa janelas, mas passamos total de velas.)
    # 5 velas/minutp em 6h = 360/dia. Para 7 dias, 2500-3000 velas é seguro.
    ...
    print("\n==================================================")
    print("RELATORIO AGRESSIVO 7 DIAS (DADOS MT5)")
    print("==================================================")
    ...
    if backtester.trades:
        print("=== LISTAGEM DE TRADES (7 DIAS) ===")
        for t in backtester.trades:
            print(f"Data: {t['exit_time'].date()} | Horário: {t['exit_time'].time()} | Side: {t['side']} | Pts: {t['pnl_points']:.1f} | R$: {t['pnl_fin']:.2f} | Motivo: {t['reason']}")
    else:
        print("Nenhum trade realizado no período (Sniper Mode Ativo).")
    
    print("\n✅ Teste 7 Dias MT5 Concluído.")

if __name__ == "__main__":
    asyncio.run(run_aggressive_mt5_7day_test())
