...
class LeveragedBacktester30(BacktestPro):
    async def load_data(self):
        logging.info(
            f"📥 Coletando dados de 30 DIAS (~12.000-15.000 candles M1) para {self.symbol}..."
        )
        ...
        rates = await asyncio.to_thread(
            mt5.copy_rates_from_pos, self.symbol, mt5.TIMEFRAME_M1, 0, 20000
        )
        ...
        logging.info(
            f"✅ {len(df)} candles carregados. Início: {df.index[0]} | Fim: {df.index[-1]}"
        )
        return df
...
async def run_leveraged_30day():
    ...
    if backtester.trades:
        ...
        # Drawdown simples
        peaks = (
            pd.Series([1000] + [t["pnl_fin"] for t in backtester.trades])
            .cumsum()
            .expanding()
            .max()
        )
