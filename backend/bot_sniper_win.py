...
class SniperBotWIN:
    ...
    async def execute_trade(self, side, quantile_confidence="NORMAL", tp_multiplier=1.0, current_atr=None, regime=None):
        ...
        if self.risk.dry_run:
            logger.info(f"🧪 [SIMULAÇíO] LIMIT {side.upper()} Sniper disparado @ {limit_price}")
            self.trade_count += 1
            self._save_state()
            return True
        ...
        if result and result.retcode == self.bridge.mt5.TRADE_RETCODE_DONE:
            ...
            for _ in range(iterations):
                ...
                if status == "FILLED":
                    self.trade_count += 1
                    self._save_state()
                    logger.info(f"🎯 [EXECUÇíO] {side.upper()} @ {limit_price} | Trade {self.trade_count}/{self.risk.daily_trade_limit}")
                    return True
    ...
    async def manage_trailing_stop(self):
        ...
        for pos in positions:
            ...
            if v_ok:
                ...
                if self.risk.dry_run:
                    logger.info(f"🧪 [SIMULAÇíO] Fechamento {pos.ticket} por Limite de Velocidade")
                else:
    ...
    async def run(self):
        ...
        logger.info(f"Símbolo alvo: {self.symbol} | MODO: {'SIMULAÇíO' if self.risk.dry_run else 'REAL'}")
        ...
        while self.running:
            try:
                ...
                if buy_cond or sell_cond:
                    ...
                    if flux_ok:
                        # [FASE 27] DECISíO COMPLETA DA IA (Sincronizada)
