...
class SniperBotWIN:
    ...
    def calculate_vwap_and_bands(self, df):
        """Calcula VWAP e Bandas de Volatilidade (Desvio Padrão) intradiário."""
        if df is None or df.empty:
            return None, None, None
        try:
            # VWAP Simplificado (Ponderado por Volume do data_60)
            tp = (df['high'] + df['low'] + df['close']) / 3
            vwap = (tp * df['tick_volume']).sum() / df['tick_volume'].sum()
            std = df['close'].std()
            return vwap, vwap + (2.0 * std), vwap - (2.0 * std)
        except Exception:
            return None, None, None
