...
async def run_production_audit():
    ...
    print(
        f"Capital: R$ {initial_capital:.2f} | Lotes: 3.0 | SOTA Trailing: {params['trailing_trigger']}pts"
    )
    print(
        f"Breakeven: {params['be_trigger']}pts | Flux Filter: {params['vol_spike_mult']}x"
    )
    print("=" * 85 + "\n")
    ...
    if (
        ...
        and current_time < datetime.strptime("17:45", "%H:%M").time()
    ):
    ...
    print(
        f"📅 Dia Analisado: {last_day.strftime('%d/%m/%Y')} ({len(day_data)} candles M1)"
    )
    ...
    if total_pnl > 0:
        ...
        print(
            "⚠️ Configuração defensiva. Possível preservação de capital em dia difícil."
        )

    if shadow["filtered_by_ai"] > 5:
        print(
            "💡 Sugestão: Reduzir Confidence Threshold para 0.80 pode aumentar trades, mas aumenta risco."
        )

    if total_pnl > 0 and wr < 50:
        print(
            "💡 Sugestão: O Trailing Stop de 70pts salvou a operação. Manter ativado."
        )
