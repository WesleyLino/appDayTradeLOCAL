...
logger = logging.getLogger("TesteFocado19Fev")


async def test_focused_19feb():
    logger.info("🤖 [SOTA-v24] Iniciando Teste Focado de 1 Dia: 19/02/2026")
    ...
    # Datas: Pegamos o dia 19/02 e o dia anterior para Warm-up
    ...
    # Como o backtest é baseado em M1 (sem ticks), usamos proxies matemáticos para auditoria de 1 dia:
    ...
    df_rates["trap_index"] = (
        0.0  # Neutro para backtest sem fluxo real (v22 style metrics fallback)
    )
    ...
    # Carregar SOTA v24 (Prioridade v24_locked_params.json)
    ...
    # Calibragem v24.4: Threshold de confiança relaxado para auditoria (60%)
    bt.opt_params["confidence_level"] = sota_params.get("confidence_threshold", 0.60)
    bt.opt_params["enable_news_filter"] = False

    # 3. Executar
    logger.info("🚀 Executando simulação focado no dia 19/02...")
    await bt.run()
    ...
    logger.info("\n" + "=" * 60)
    logger.info("🏆 RESULTADO FOCADO - 19/02/2026")
...
if __name__ == "__main__":
    asyncio.run(test_focused_19feb())
