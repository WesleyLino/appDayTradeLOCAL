...
_LOG_FILE = os.path.join(
    os.path.dirname(__file__), "..", "backend", "audit_20fev_v2_result.txt"
)
...
async def _run_scenario(
    day_chunk: object, capital: float, label: str, **params
) -> dict:
...
def _print_detail(r: dict):
    ...
    print(f"{'=' * 90}")
    print(
        f"  P&L Total   : R$ {r['pnl']:+.2f}  |  Win Rate: {r['wr']:.1f}%  ({r['wins']}V/{r['losses']}D)"
    )
    print(f"  Drawdown    : {r['max_dd']:.2f}%  |  Profit Factor: {r['pf']:.2f}")
    print(
        f"  Shadow AI   : {r['shadow_ai']} sinais  |  Shadow FLUX: {r['shadow_flux']} sinais"
    )
    if r["trade_list"]:
...
def _print_comparison(a: dict, b: dict):
    ...
    print(
        fmt.format(
            "Win Rate",
            f"{a['wr']:.1f}%",
            f"{b['wr']:.1f}%  {diff(b['wr'], a['wr'], pct=True)}",
        )
    )
    print(
        fmt.format(
            "Drawdown Max",
            f"{a['max_dd']:.2f}%",
            f"{b['max_dd']:.2f}%  {diff(b['max_dd'], a['max_dd'], pct=True)}",
        )
    )
...
async def run_validation():
    ...
    print("\n" + "=" * 90)
    print(
        f"  RAIO-X COMPARATIVO: WIN$ | {TARGET_DATE.strftime('%d/%m/%Y')} | R$ {initial_capital:.2f}"
    )
    print(
        f"  Trailing: {base_params['trailing_trigger']}pts | Flux: {base_params['vol_spike_mult']}x"
    )
    ...
    result_a = await _run_scenario(
        day_chunk, initial_capital, "A: ATUAL (BE=50)", **params_a
    )
    ...
    result_b = await _run_scenario(
        day_chunk, initial_capital, "B: MELHORIA (BE=40 + 09:15)", **params_b
    )
    ...
    if gained_b > gained_a:
        print(
            f"  BE=40pts protegeu melhor o capital: +R$ {gained_b - gained_a:.2f} vs atual."
        )
    elif gained_b <= gained_a:
        print("  Neste dia, o BE=50pts foi mais permissivo e capturou mais ganho.")
        print(
            "  Sugestao: Manter BE=50 por enquanto. Testar BE=40 em mais dias antes de alterar producao."
        )
