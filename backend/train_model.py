...
class PatchTST(nn.Module):
    def __init__(self, c_in, context_window, target_window, patch_len, stride, n_layers, n_heads, d_model, n_quantiles):
        ...
        self.encoder = nn.Sequential(*[TransformerBlock(d_model, n_heads, d_model*4) for _ in range(n_layers)])
...
def load_sota_data(data_dir="data/sota_training/"):
    ...
    if win_cols: m = m[win_cols + [c for c in m.columns if c not in win_cols]]
...
def train():
    ...
    for e in range(5):
        ...
        for i, (x, y) in enumerate(loader):
            ...
            loss.backward()
            o.step()
            if i % 100 == 0: logging.info(f"E{e+1} B{i}/{len(loader)} L:{loss.item():.4f}")
