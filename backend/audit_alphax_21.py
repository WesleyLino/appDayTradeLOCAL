...
class AlphaXBacktest(BacktestPro):
    ...
    def simulate_oco(self, row, position):
        ...
        current_pnl_pts = (
            (row["close"] - entry) if side == "buy" else (entry - row["close"])
        )
        ...
        if current_pnl_pts <= self.velocity_drawdown_limit:
            ...
            return "VELOCITY_LIMIT", (
                entry + self.velocity_drawdown_limit
            ) if side == "buy" else (entry - self.velocity_drawdown_limit)
...
async def run_audit():
    ...
    logging.info(
        "📥 Conectando ao MetaTrader 5 para coleta de histórico especializado..."
    )
    ...
    if full_df is None or full_df.empty:
        logging.error(
            "❌ Falha crítica: Verifique se o MT5 está conectado e o símbolo WIN$ visível."
        )
        return
    ...
    for date_str in target_dates:
        ...
        tester = AlphaXBacktest(symbol=symbol, initial_balance=capital, base_lot=1)
    ...
    output_report += (
        f"- **Retorno sobre Capital**: {(total_pnl / capital * 100):.2f}%\n"
    )
