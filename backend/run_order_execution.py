...
async def main():
    ...
    if valid_symbols:
        logging.info(
            f"Testando tick de cotação real nos ativos vigentes: {valid_symbols}"
        )
    ...
    if not tick:
        logging.error(
            f"Erro ao obter tick para todos os ativos testados (incluindo {symbol_test}). Terminal aberto no ativo com cotações chegando?"
        )
        return
    ...
    if result.retcode == bridge.mt5.TRADE_RETCODE_DONE:
        ...
        logging.warning(
            "⏳ Aguardando 3 segundos de teste de mercado antes do Panic Close..."
        )
