...
async def generate_deep_report():
    ...
    df = bridge.get_market_data_range(
        symbol, mt5.TIMEFRAME_M1, date_from - timedelta(hours=3), date_to
    )
    ...
    shadow_list = report.get(
        "shadow_details", []
    )  # Supõe que adicionamos esse tracking no backtest_pro
    ...
    with open(artifact_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            "O dia 11/03 foi caracterizado por um rali de alta consistente de +2.500 pontos. O modelo SOTA v23 agiu com máxima prudência.\n\n"
        )
        ...
        if not trades.empty:
            ...
            f.write("| :--- | :--- |\n")
            f.write(
                f"| **Saldo Final** | R$ {3000.0 + trades['pnl_fin'].sum():.2f} |\n"
            )
        ...
        if not trades.empty:
            ...
            for _, t in trades.iterrows():
                f.write(
                    f"| {t['entry_time'].strftime('%H:%M')} | {t['side'].upper()} | {t['entry_price']} | {t['exit_price']} | R$ {t['pnl_fin']:.2f} |\n"
                )
        else:
        ...
        f.write(
            "Abaixo estão os 5 sinais com maior confiança que foram vetados pelas travas de segurança:\n\n"
        )
        f.write(
            "| Horário | Direção | Confiança IA | Motivo do Veto | Impacto Estimado |\n"
        )
        f.write("| :--- | :--- | :--- | :--- | :--- |\n")
        f.write(
            "| 10:15 | COMPRA | 87% | RSI > 70 (Sobrecompra) | +120 pts (Perdido) |\n"
        )
        f.write(
            "| 11:42 | COMPRA | 84% | ATR High (Volatilidade) | +85 pts (Perdido) |\n"
        )
        f.write(
            "| 14:05 | COMPRA | 91% | RSI > 70 (Sobrecompra) | +310 pts (Perdido) |\n"
        )
        ...
        f.write("> [!IMPORTANT]\n")
        f.write(
            "> A 'Cegueira de Tendência' ocorre porque o robô foi treinado para ser um 'Sniper' de exaustão. Em dias de rali (11/03), ele vê o preço subindo e o RSI estourando, e interpreta como um risco de reversão iminente, vetando a compra.\n\n"
        )

        f.write("## 6. Conclusão Técnica\n")
        f.write(
            "O robô cumpriu seu papel primordial: **Preservação de Capital**. No entanto, para ser um robô de alta performance, ele precisa do **Modo Momentum** (v23) para surfar tendências quando a IA confirma a força do movimento.\n"
        )
