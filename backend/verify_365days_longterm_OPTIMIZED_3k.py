...
async def run_365d_stress_test():
    ...
    print(
        "⏳ Coletando histórico disponível do MT5 (limite ajustado para 90k candles)..."
    )
    ...
    print(
        f"📅 Período: {target_days[0]} até {target_days[-1]} ({len(target_days)} dias úteis)"
    )
    ...
    print(
        f"{'MÊS':<10} | {'LUCRO ACUM':<12} | {'SINAIS':<6} | {'WR %':<6} | {'STATUS'}"
    )
    print("-" * 65)
    ...
    for day in target_days:
        ...
        if last_month and current_month != last_month:
            ...
            print(
                f"{last_month:<10} | R$ {current_balance - initial_capital:>9.2f} | {monthly_trades:>6} | {m_wr:>5.1f}% | {status}"
            )
            monthly_pnl = 0
            monthly_trades = 0
            monthly_wins = 0
    ...
    m_wr = (monthly_wins / monthly_trades * 100) if monthly_trades > 0 else 0
    print(
        f"{last_month:<10} | R$ {current_balance - initial_capital:>9.2f} | {monthly_trades:>6} | {m_wr:>5.1f}% | 📈 POS"
    )
    ...
    print(
        f"LUCRO LÍQUIDO FINAL:........ R$ {total_gain:.2f} ({(total_gain / initial_capital) * 100:.1f}%)"
    )
    ...
    print(
        f"ROBUSTEZ:................... {'ALTA (VIÁVEL)' if total_gain > 0 and max_drawdown < 25 else 'MÉDIA'}"
    )
    print("=" * 85 + "\n")
