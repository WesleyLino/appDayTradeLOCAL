...
async def test_20feb_sota_final():
    ...
    if not os.path.exists(params_file):
        logger.error(f"❌ Arquivo de parâmetros não encontrado!")
        return
    ...
    try:
        ...
        logger.info(f"✅ Configurações carregadas de {params_file} (Confiança: {sota_params.get('confidence_threshold')})")
    ...
    logger.info(f"📊 Processando simulação para {symbol} (Dia 20/02) em modo SOTA v24...")
    await bt.run()
    ...
    logger.info(f"\n" + "="*50)
    logger.info(f"🏆 RESULTADO DIA 20/02 (SOTA v24)")
    logger.info(f"==================================================")
