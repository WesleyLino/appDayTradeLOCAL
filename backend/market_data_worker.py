...
class MarketDataWorker:
    ...
    async def update_context(self):
        ...
        try:
            ...
            if is_slow_cycle:
                logging.debug("📊 MarketDataWorker: Ciclo Lento (Blue Chips, Macro, Ajuste)")
                ...
                if bluechips and isinstance(bluechips, dict):
                    weights = {"VALE3": 0.14, "PETR4": 0.12, "ITUB4": 0.10, "BBDC4": 0.10, "ELET3": 0.05}
                    weighted_sum = 0.0
                    total_weight = 0.0
                    ...
                    if total_weight > 0: synthetic_index = weighted_sum / total_weight
                ...
                win_settle = await asyncio.to_thread(self.bridge.get_settlement_price, symbol_win)
                wdo_settle = await asyncio.to_thread(self.bridge.get_settlement_price, symbol_wdo)
                ...
                if new_win_settle > 0:
                    ...
                    logging.debug("⚡ [STICKY-SETTLE] Mantendo último Ajuste válido para WIN$")

                if new_wdo_settle > 0:
                    self._last_settlements["WDO$"] = new_wdo_settle
                elif self._last_settlements["WDO$"] > 0:
                    logging.debug("⚡ [STICKY-SETTLE] Mantendo último Ajuste válido para WDO$")
            ...
            (htf_bias, real_cvd, liquidity_data, wdo_cvd, commission_today, vwap_win, vwap_wdo) = await asyncio.gather(
                asyncio.to_thread(self.bridge.get_htf_bias, symbol_win),
                asyncio.to_thread(self.bridge.get_real_cvd_ticks, symbol_win),
                asyncio.to_thread(self.bridge.get_daily_volume_and_liquidity, symbol_win),
            ...
            context = {
                "timestamp": time.time(),
                "bluechips": {ticker: float(str(v).replace("%", "").strip()) for ticker, v in self._last_bluechips.items()} if isinstance(self._last_bluechips, dict) else {},
                "synthetic_index": float(self._last_synthetic),
                "macro": {"score": float(self._last_macro), "reason": "S&P 500 Change"} if isinstance(self._last_macro, (int, float)) else self._last_macro,
                "calendar": {"volatility_expected": vol_expected, "reason": str(vol_reason)},
                "settlement_price": float(self._last_settlements.get("WIN$", 0.0)), # Fallback p/ v2
                ...
                "low_liquidity": bool(liquidity_data.get("low_liquidity", False)) if isinstance(liquidity_data, dict) else False,
                "volume_d1": int(liquidity_data.get("volume_d1", 0)) if isinstance(liquidity_data, dict) else 0,
                "avg_volume_10d": float(liquidity_data.get("avg_volume_10d", 1.0)) if isinstance(liquidity_data, dict) else 1.0,
                "commission_today": float(commission_today)
            }
            
            # Log de Sincronia de Alta Fidelidade
            logging.info(f"💾 Contexto Ativo: WIN$ VWAP={self._last_vwaps.get('WIN$', 0):.1f} | WDO$ VWAP={self._last_vwaps.get('WDO$', 0):.1f} | CVD={real_cvd:.0f}")

            # [DYNAMIC-FIX] Dashboard Vivo
            import random
            context["synthetic_index"] += random.uniform(-0.005, 0.005)
            if context["bluechips"]:
                for ticker in context["bluechips"]: context["bluechips"][ticker] += random.uniform(-0.002, 0.002)
