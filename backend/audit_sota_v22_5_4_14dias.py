...
async def rodar_auditoria_14dias():
    ...
    console.info(
        f"   Capital: R$ {initial_capital:.2f} | Ativo: {symbol} | Timeframe: {timeframe}"
    )
    console.info("=" * 70)
    ...
    for data_alvo in datas:
        ...
        if day_data.empty:
            console.warning(
                f"   ⚠️  Sem dados para {data_str} (feriado ou dado ausente)."
            )
        ...
        veto_h1 = shadow.get("veto_reasons", {}).get(
            "VETO_TENDENCIA_BAIXA_H1", 0
        ...
        console.info(
            f"   {icone}  PnL={pnl_dia:+.2f} | Trades={len(trades)} ({len(compras)}C/{len(vendas)}V) | WR={wr:.1f}%"
        )
    ...
    md.append(
        "| Data | PnL do Dia | Trades | 🟢 Compra | 🔴 Venda | Win Rate | Oport. Perdidas (IA/Flux) | Veto H1 | Saldo |"
    )
    md.append(
        "|:-----|:----------:|:------:|:---------:|:--------:|:--------:|:-------------------------:|:-------:|------:|"
    )
    ...
    md.append(
        f"| 📈 Retorno sobre Capital | **{(pnl_acumulado / initial_capital * 100):+.1f}%** |"
    )
    md.append(f"| 📅 Dias Positivos | {dias_positivos} / {len(dias_validos)} |")
    md.append(f"| 📅 Dias Negativos | {dias_negativos} / {len(dias_validos)} |")
    md.append(
        f"| 🎯 Win Rate Global | {wr_global:.1f}% ({total_wins_acum}W/{total_losses_acum}L) |"
    )
    md.append(
        f"| 📊 Total de Trades | {total_trades_acum} ({total_compras}C / {total_vendas}V) |"
    )
    ...
    for r in resultados_dia:
        ...
        md.append(
            f"- **Operações Compradas**: {r['compras']} trades → PnL R$ {r['pnl_c']:+.2f}"
        )
        md.append(
            f"- **Operações Vendidas**: {r['vendas']} trades → PnL R$ {r['pnl_v']:+.2f}"
        )
        md.append(
            f"- **Win Rate**: {r['win_rate']:.1f}% ({r['wins']}W / {r['losses']}L)"
        )
        md.append(
            f"- **Shadow — Oport. Perdidas por IA**: {r['oport_ia']} entradas vetadas por baixa confiança"
        )
        md.append(
            f"- **Shadow — Oport. Perdidas por Fluxo**: {r['oport_flux']} entradas vetadas por fluxo insuficiente"
        )
        ...
        if r["pnl"] < -50:
            alertas.append(
                "⚠️ **Drawdown elevado**: Revisar se o Sell-Only H1 estava ativo durante a queda."
            )
        if r["pnl_c"] < -30 and r["veto_h1"] > 0:
            alertas.append(
                "🛡️ **Proteção Sell-Only**: Vetos H1 ativos — compras que passaram podem ter sido contra tendência residual."
            )
        if r["oport_ia"] > 10:
            alertas.append(
                f"🔍 **Alta filtragem IA**: {r['oport_ia']} oportunidades vetadas. Avaliar se o `confidence_relax_factor` deve ser ajustado."
            )
        if r["oport_flux"] > 10:
            alertas.append(
                f"📉 **Alta filtragem de Fluxo**: {r['oport_flux']} vetos por volume insuficiente. Mercado lateral/fraco."
            )
        if r["win_rate"] > 75 and r["trades"] >= 3:
            alertas.append(
                "🏆 **Dia de alto rendimento**: Win Rate > 75%. Candidato a aumentar lote dinâmico."
            )
        if not alertas:
    ...
    md.append("|:-----|:------------:|:-------------:|:--------------:|")
    contrib_c = (
        (pnl_compras_total / abs(pnl_acumulado) * 100) if pnl_acumulado != 0 else 0
    )
    contrib_v = (
        (pnl_vendas_total / abs(pnl_acumulado) * 100) if pnl_acumulado != 0 else 0
    )
    md.append(
        f"| 🟢 COMPRA | {total_compras} | R$ {pnl_compras_total:+.2f} | {contrib_c:+.1f}% |"
    )
    md.append(
        f"| 🔴 VENDA  | {total_vendas} | R$ {pnl_vendas_total:+.2f} | {contrib_v:+.1f}% |"
    )
    ...
    md.append(
        f"- **Total de oportunidades vetadas pela IA** (baixa confiança): **{total_ia}**"
    )
    md.append(f"- **Total de oportunidades vetadas por fluxo**: **{total_flux}**")
    md.append(
        f"- **Total de vetos H1** (proteção anti-contratendência V22.5.4): **{total_h1}**"
    )
    md.append("")
    ...
    if total_ia > 50:
        md.append(
            f"1. **Confidence Relax Factor**: Com {total_ia} vetos, considerar reduzir `confidence_relax_factor` de 0.80 para 0.75 "
            f"para relaxar ainda mais o threshold em dias de ATR alto + H1 alinhado."
        )
    else:
        md.append(
            "1. **Confidence Threshold**: Filtragem de IA equilibrada. Manter `confidence_relax_factor = 0.80`."
        )

    if total_flux > 30:
        md.append(
            f"2. **Fluxo/Volume**: {total_flux} oportunidades vetadas por volume. Avaliar reduzir `vol_spike_mult` de 1.5 para 1.3 "
            f"em períodos de baixa liquidez (antes de 10h)."
        )
    else:
        md.append(
            "2. **Filtro de Fluxo/Volume**: Calibrado adequadamente. Manter configurações atuais."
        )

    if pnl_compras_total < 0 and pnl_vendas_total > 0:
        md.append(
            "3. **Bias Direcional**: Compras estão no vermelho enquanto vendas são lucrativas — confirma eficácia do Sell-Only H1. "
            "Considerar aumentar o rigor do filtro de compras em dias de H1 neutro."
        )
    elif pnl_vendas_total < 0 and pnl_compras_total > 0:
        md.append(
            "3. **Bias Direcional**: Vendas estão no vermelho — revisar gatilhos de venda, especialmente RSI sell level e BB superior."
        )
    else:
        md.append(
            "3. **Bias Direcional**: Ambos os lados contribuindo positivamente. Estratégia equilibrada."
        )

    if wr_global < 55:
        md.append(
            "4. **Win Rate Global abaixo de 55%**: Revisar o `adx_min_threshold` — entradas em mercados fracos podem estar diluindo o WR."
        )
    elif wr_global > 65:
        md.append(
            f"4. **Win Rate Global de {wr_global:.1f}%**: Excelente! Pode-se estudar aumentar o tamanho do lote em dias de alta volatilidade alinhada."
        )
    else:
        md.append(
            f"4. **Win Rate Global de {wr_global:.1f}%**: Dentro do target. Manter configurações V22.5.4."
        )

    md.append("\n---\n")
    md.append(
        f"*Auditoria gerada automaticamente em {datetime.now().strftime('%d/%m/%Y %H:%M')} | SOTA V22.5.4*"
    )
    ...
    console.info(
        f"  RETORNO           : {(pnl_acumulado / initial_capital * 100):+.1f}%"
    )
