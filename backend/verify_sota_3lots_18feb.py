...
class LeveragedBacktester(BacktestPro):
    async def load_data(self):
        ...
        rates = await asyncio.to_thread(mt5.copy_rates_range, self.symbol, mt5.TIMEFRAME_M1, utc_from, utc_to)
        if rates is None or len(rates) == 0:
            ...
            rates = await asyncio.to_thread(mt5.copy_rates_from_pos, self.symbol, mt5.TIMEFRAME_M1, 1200, 600)
        ...
        logging.info(f"✅ {len(df)} candles carregados (18/02). Primeiros dados: {df.index[0]}")
        return df
