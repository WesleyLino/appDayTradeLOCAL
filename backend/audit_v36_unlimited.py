...
async def run_audit_unlimited_optimized():
    ...
    for dia_str in dias:
        ...
        day_trades = [t for t in res['trades'] if (t['entry_time'].date() if hasattr(t['entry_time'], 'date') else datetime.strptime(str(t['entry_time']), "%Y-%m-%d %H:%M:%S").date()) == dia_dt]
        
        p_dia = sum(t['pnl_fin'] for t in day_trades)
        status = "🔥" if p_dia > 100 else "✅" if p_dia > 0 else "🛑" if p_dia < 0 else "─"
