...
_LOG_FILE = os.path.join(
    os.path.dirname(__file__), "..", "backend", "audit_19fev_result.txt"
)
...
async def run_audit():
    ...
    print("\n" + "=" * 90)
    print(
        f"  RAIO-X DE PRODUCAO: WIN$ | {TARGET_DATE.strftime('%d/%m/%Y')} | CAPITAL: R$ {initial_capital:.2f}"
    )
    print(
        f"  Trailing: {params['trailing_trigger']}pts | BE: {params['be_trigger']}pts | Flux: {params['vol_spike_mult']}x"
    )
    print("=" * 90 + "\n")
    ...
    if TARGET_DATE not in available_days:
        print(
            f"AVISO: Dia {TARGET_DATE} nao encontrado. Dias disponiveis: {available_days}"
        )
    ...
    print(
        f"Candles M1    : {len(target_rows)} candles do dia ({len(day_chunk)} com padding)"
    )
    ...
    pf = (
        abs(sum(t["pnl_fin"] for t in wins) / sum(t["pnl_fin"] for t in losses))
        if losses
        else float("inf")
    )
    ...
    print(
        f"  Lucro/Prejuizo   : R$ {total_pnl:+.2f}  ({'+' if total_pnl >= 0 else ''}{(total_pnl / initial_capital) * 100:.2f}%)"
    )
    ...
    if trades:
        print(
            f"  {'#':<3} {'HORA':<8} {'DIRECAO':<7} {'ENTRY':<9} {'EXIT':<9} {'PNL':>10}  STATUS"
        )
        print(f"  {'-' * 70}")
        for i, t in enumerate(trades, 1):
            hora = (
                str(t.get("entry_time", ""))[-8:-3] if t.get("entry_time") else "--:--"
            )
            ...
            print(
                f"  {i:<3} {hora:<8} {direc:<7} {entry:<9.0f} {ext:<9.0f} R$ {pnl:>8.2f}  {status}"
            )
    ...
    if total_pnl > 0:
        ...
        print(
            f"  RESULTADO: Dia de perda (R$ {total_pnl:.2f}). Analise as derrotas abaixo."
        )
    ...
    if shadow["filtered_by_flux"] > 5:
        ...
        print()
        print(
            "  [2] Filtro de Fluxo (1.2x): Dia de alta volatilidade. Captura totalmente eficiente."
        )
    ...
    if losses:
        ...
        print()
        print(
            "  [3] Trailing Stop de 70pts: Zero perdas. Configuracao IDEAL para este regime."
        )
