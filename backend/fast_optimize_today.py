...
def run_test(params, df_today):
    ...
    for i in range(len(df_today)):
        ...
        if not bt.position:
            rsi = row['rsi']
            if rsi < 30 and row['close'] < row['lower_bb'] and row['tick_volume'] > (row['vol_sma'] * params['vol_spike_mult']):
                side = "buy"
            elif rsi > 70 and row['close'] > row['upper_bb'] and row['tick_volume'] > (row['vol_sma'] * params['vol_spike_mult']):
                side = "sell"
            else: side = None
...
async def fast_optimize_today_strict():
    ...
    if best_results:
        ...
        with open('best_params_WIN.json', 'w') as f:
            json.dump({"params": params, "net_profit": pnl, "trades": trades}, f, indent=4)
