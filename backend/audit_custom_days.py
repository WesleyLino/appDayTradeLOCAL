...
async def run_custom_audit():
    ...
    logging.info(
        f"🚀 INICIANDO AUDITORIA CUSTOMIZADA SOTA V10.0 MAESTRO III ({symbol})"
    )
    ...
    for target_date in dates_to_test:
        ...
        detailed_reports += (
            f"- Falta de Confiança IA (Oportunidades vetadas < 81% conf): {missed_ai}\n"
        )
        detailed_reports += (
            f"- Falta de Pressão de Fluxo (Oportunidades vetadas): {missed_flux}\n"
        )
