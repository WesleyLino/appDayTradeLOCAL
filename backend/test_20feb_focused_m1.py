...
logger = logging.getLogger("TesteFocado20Fev")

async def test_focused_20feb():
    logger.info("🤖 [SOTA-v24] Iniciando Teste Focado de 1 Dia: 20/02/2026")
    ...
    # Datas: Pegamos o dia 20/02 e o dia anterior para Warm-up
    date_from = datetime(2026, 2, 19, 9, 0) 
    date_to = datetime(2026, 2, 20, 18, 0)
    ...
    # Como o backtest é baseado em M1 (sem ticks), usamos proxies matemáticos:
    ...
    df_rates['trap_index'] = 0.0 # Neutro para backtest sem fluxo real
    ...
    # [PATCH] Injeta o DataFrame diretamente para evitar nova coleta pelo BT
    bt.data = df_rates
    
    # Sobrescreve load_data para não fazer nada (já temos os dados)
    ...
    # 3. Executar
    logger.info("🚀 Executando simulação focado no dia 20/02...")
    # Chamamos o core do backtest diretamente para evitar o .run() que chama load_data() internamente
    # No entanto, como patcheamos load_data, o .run() deve funcionar agora.
    await bt.run()
    ...
    logger.info("\n" + "="*60)
    logger.info(f"🏆 RESULTADO FOCADO - 20/02/2026")
...
if __name__ == "__main__":
    asyncio.run(test_focused_20feb())
