...
async def run_detailed_audit_09mar():
    ...
    print(
        "Foco: Mini Índice (WIN$) | Capital: R$ 3.000,00 | Lotes: 3 (Base) + Dinâmico"
    )
    print("=" * 80 + "\n")
    ...
    trades = [
        ...
        if pd.to_datetime(t["exit_time"]).date() == target_date
    ]
    ...
    if veto_reasons:
        ...
        print(
            "Nenhuma oportunidade perdida significativa ou dados de shadow não processados."
        )
    ...
    if pnl_total > 0:
        ...
        print(
            "2. Reduzir o Trailing Step para 10pts em dias de ATR > 400 pode travar ainda mais lucro."
        )
    else:
        print("1. O mercado de hoje apresentou reversões bruscas.")
        print(
            "2. Recomenda-se manter o Breakeven em 70pts para evitar saídas prematuras em pullbacks."
        )
