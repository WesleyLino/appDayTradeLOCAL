...
async def run_deep_potential_analysis():
    ...
    if df is not None:
        ...
        print("=" * 70)
        print(
            f"Lucro Líquido:.............. R$ {report['total_pnl']:.2f} ({(report['total_pnl'] / initial_capital) * 100:.1f}%)"
        )
        ...
        if report["win_rate"] >= 80:
            print(
                "- Assertividade EXCELENTE. Sugestão: Aumentar volume progressivamente via Anti-Martingale."
            )
        elif shadow.get("filtered_by_ai", 0) > 5:
            print(
                "- Muitos sinais bloqueados pela IA. Sugestão: Ajustar 'confidence_threshold' para 0.80 para capturar mais volume."
            )
        else:
            print(
                "- Perfil conservador mantido. Sugestão: Refinar STOP loss dinâmico baseado no ATR para evitar ruído."
            )
        ...
        print(
            "- Considerar filtro de 'Sentiment' agressivo para evitar contratendências macro."
        )
        print("=" * 70 + "\n")
