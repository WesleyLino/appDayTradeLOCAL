...
async def run_ultra_audit():
    ...
    print(f"{'DATA':<12} | {'PNL COMPRA':<12} | {'PNL VENDA':<12} | {'TOTAL':<10} | {'TRADES'} | {'MISS'}")
    print("-" * 95)
    ...
    for date_str in target_dates_str:
        ...
        print(f"{day.strftime('%d/%m/%Y'):<12} | R$ {pnl_buy:>9.2f} | R$ {pnl_sell:>9.2f} | R$ {total:>8.2f} | {len(trades):^6} | {missed:^4}")
