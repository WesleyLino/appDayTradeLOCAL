...
async def run_final_audit():
    ...
    try:
        ...
        print("="*80)
        print(f"Capital: R$ {capital:.2f} | PnL: R$ {total_pnl:.2f} ({(total_pnl/capital*100):+.2f}%)")
        print(f"Trades Totais: {total_trades}")
        
        if total_trades > 0:
            print(f"{'HORA':<8} | {'LADO':<8} | {'PNL':<10} | {'JANELA':<10} | {'MOTIVO':<20}")
            print("-" * 80)
            for t in bt.trades:
                exit_t = t['exit_time']
                hora_str = exit_t.strftime('%H:%M') if hasattr(exit_t, 'strftime') else str(exit_t)
                janela = "NORMAL"
                # Heurística para Janela de Ouro
                if exit_t.hour == 10 or exit_t.hour == 11: 
                    janela = "GOLDEN"
                
                print(f"{hora_str:<8} | {t['side']:<8} | R$ {t['pnl']:<8.2f} | {janela:<10} | {t.get('exit_reason', 'N/A'):<20}")
        else:
