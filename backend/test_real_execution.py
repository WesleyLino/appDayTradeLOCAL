...
async def test_execution_flow():
    ...
    print(f"🕒 Horário permitido agora: {'SIM' if time_allowed else 'NÃO (Veto Ativo)'}")

    # B. Filtro Macro (Blue Chips)
    macro_allowed = risk.is_macro_allowed("BUY", 0.05) # Simula Blue Chips subindo 0.05%
    print(f"🛑 Filtro Macro (Compra com Blue Chips +0.05%): {'LIBERADO' if macro_allowed else 'VETADO'}")
    ...
    print(f"📡 Ambiente (Ping: {ping}ms, Spread: {spread}): {'OK' if env_ok else 'ERRO: ' + env_msg}")
    ...
    if tick:
        ...
        print(f"📦 Payload OCO Gerado: Preço {params['price']}, SL {params['sl']}, TP {params['tp']}, Volume {params['volume']}")
        
        if params['price'] > 0 and params['sl'] < params['price'] and params['tp'] > params['price']:
    ...
    print("O MT5Bridge está configurado com lógica resiliente para B3 (Requotes/Slippage).")
