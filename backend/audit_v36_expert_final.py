...
async def run_final_audit_expert():
    ...
    print(
        f"{'Data':<12} | {'Símbolo':<7} | {'PnL Total':<10} | {'Trades':<6} | {'Regime Med':<10} | {'Status'}"
    )
    print("-" * 80)
    ...
    for dia_str in dias:
        ...
        print(
            f"{dia_str:<12} | {'WIN$':<7} | R$ {p_dia:>8.2f} | {len(day_trades):^6} | {regime_desc:^10} | {status}"
        )
    ...
    print(
        f"   >>> Profit Factor Estimado: {(totais['compra'] + totais['venda']) / abs(min(-1, (totais['compra'] if totais['compra'] < 0 else 0) + (totais['venda'] if totais['venda'] < 0 else 0))):.2f}"
    )
