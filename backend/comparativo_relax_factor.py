...
async def rodar_cenario(
    full_data, symbol, capital_inicial, strategy_params, relax_factor, label
):
...
async def comparar_fatores():
    ...
    if full_data is None or full_data.empty:
        ...
        return
    console.info(
        f"✅ {len(full_data)} candles carregados. Iniciando simulação dupla...\n"
    )

    # Roda os dois cenários sequencialmente (mesmo full_data)
    console.info("🔷 Cenário A: fator = 0.80 (base V22.5.4)...")
    cenario_a = await rodar_cenario(
        full_data, symbol, capital, strategy_params, 0.80, "V22.5.4 (fator=0.80)"
    )

    console.info("🔶 Cenário B: fator = 0.75 (teste)...")
    cenario_b = await rodar_cenario(
        full_data, symbol, capital, strategy_params, 0.75, "V22.5.4-B (fator=0.75)"
    )
    ...
    md.append(
        f"| 💰 PnL Total | **R$ {cenario_a['pnl_total']:+.2f}** | **R$ {cenario_b['pnl_total']:+.2f}** | {delta_pnl:+.2f} |"
    )
    md.append(
        f"| 🏦 Saldo Final | R$ {cenario_a['saldo_final']:.2f} | R$ {cenario_b['saldo_final']:.2f} | {delta_pnl:+.2f} |"
    )
    md.append(
        f"| 📈 Retorno | {cenario_a['retorno_pct']:+.1f}% | {cenario_b['retorno_pct']:+.1f}% | {delta_pnl / capital * 100:+.1f}pp |"
    )
    md.append(
        f"| 🎯 Win Rate | {cenario_a['wr']:.1f}% | {cenario_b['wr']:.1f}% | {delta_wr:+.1f}pp |"
    )
    md.append(
        f"| 📊 Total de Trades | {cenario_a['trades']} | {cenario_b['trades']} | {delta_trades:+d} |"
    )
    md.append(
        f"| ✅ Dias Positivos | {cenario_a['dias_pos']}/14 | {cenario_b['dias_pos']}/14 | {cenario_b['dias_pos'] - cenario_a['dias_pos']:+d} |"
    )
    md.append(
        f"| ❌ Dias Negativos | {cenario_a['dias_neg']}/14 | {cenario_b['dias_neg']}/14 | {cenario_b['dias_neg'] - cenario_a['dias_neg']:+d} |"
    )
    ...
    for ra, rb in zip(cenario_a["resultados"], cenario_b["resultados"]):
        ...
        md.append(
            f"| {ra['data']} | {sinal} R$ {ra['pnl']:+.2f} | {ra['trades']} | {sinal_b} R$ {rb['pnl']:+.2f} | {rb['trades']} | {d_icon} {delta:+.2f} |"
        )
    ...
    if delta_pnl > 0:
        md.append(
            f"✅ **Fator 0.75 é MELHOR**: Ganho adicional de R$ {delta_pnl:+.2f} nos 14 dias."
        )
        md.append(f"   - {delta_trades:+d} trades adicionais foram executados.")
        if delta_wr >= 0:
            md.append(
                f"   - Win Rate melhorou {delta_wr:+.1f}pp → as novas entradas são de qualidade."
            )
            md.append(
                "   - **Recomendação**: Atualizar `confidence_relax_factor = 0.75` no JSON."
            )
        else:
            md.append(
                f"   - Win Rate piorou {delta_wr:.1f}pp → mais trades, mas menor precisão."
            )
            md.append(
                "   - **Recomendação**: Avaliar trade-off PnL×WR antes de aplicar."
            )
    elif delta_pnl < 0:
        md.append(
            f"⚠️ **Fator 0.80 é MELHOR**: Fator 0.75 gerou {delta_pnl:.2f} de resultado inferior."
        )
        md.append(
            "   - As entradas adicionais liberadas com 0.75 não foram lucrativas."
        )
        md.append(
            "   - **Recomendação**: Manter `confidence_relax_factor = 0.80` (V22.5.4 original)."
        )
    else:
        md.append(
            "➡️ **Resultados iguais**: O fator não gerou diferença nos 14 dias testados."
        )
    ...
    for k, v in [("🔷 V22.5.4 (0.80)", cenario_a), ("🔶 Teste (0.75)", cenario_b)]:
        console.info(
            f"  {k}: PnL={v['pnl_total']:+.2f} | WR={v['wr']:.1f}% | Trades={v['trades']} | Dias+={v['dias_pos']}/14"
        )
    console.info(
        f"  Δ PnL : {delta_pnl:+.2f} | Δ Trades: {delta_trades:+d} | Δ WR: {delta_wr:+.1f}pp"
    )
    console.info(f"{'=' * 70}")
