...
def test_dynamic_atr():
    ...
    atr_low = 80.0
    params_low = risk.get_order_params(
        "WIN$", mt5.ORDER_TYPE_BUY, 120000, 1, current_atr=atr_low
    )
    print(f"\n--- ATR BAIXO ({atr_low}) ---")
    ...
    atr_norm = 120.0
    params_norm = risk.get_order_params(
        "WIN$", mt5.ORDER_TYPE_BUY, 120000, 1, current_atr=atr_norm
    )
    print(f"\n--- ATR NORMAL ({atr_norm}) ---")
    ...
    atr_high = 400.0
    params_high = risk.get_order_params(
        "WIN$", mt5.ORDER_TYPE_BUY, 120000, 1, current_atr=atr_high
    )
    ...
    # Simulação WDO (Dólar)
    atr_wdo = 5.0
    params_wdo = risk.get_order_params(
        "WDO$", mt5.ORDER_TYPE_BUY, 5400, 1, current_atr=atr_wdo
    )
    print(f"\n--- ATR WDO ({atr_wdo}) ---")
    ...
    atr_wdo_high = 20.0
    params_wdo_h = risk.get_order_params(
        "WDO$", mt5.ORDER_TYPE_BUY, 5400, 1, current_atr=atr_wdo_high
    )
