...
async def run_official_backtest():
    ...
    logger.info(f"🚀 Executando Simulação SOTA em {symbol} | Capital: R$ {bt.initial_balance}...")
    await bt.run()
    ...
    logger.info(f"\n==================================================")
    logger.info(f"📊 RESULTADO FINAL - 13/03/2026")
    logger.info(f"==================================================")
    ...
    if len(today_trades) > 0:
        ...
        logger.info(f"🎯 Assertividade: {(len(win_trades)/len(today_trades))*100:.1f}%")
    
    # Detalhar trades
    for i, t in enumerate(today_trades, 1):
        logger.info(f"   Trade {i}: {t['side'].upper()} @ {t['entry_price']} -> Saída: {t['exit_price']} | PnL: R$ {t['pnl_fin']:.2f} ({t['reason']})")

    logger.info(f"==================================================")
    logger.info(f"📁 Relatório Visual Completo em: backend/backtest_report.html")
