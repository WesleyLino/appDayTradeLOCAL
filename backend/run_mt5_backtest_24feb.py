...
async def run_mt5_analysis_24feb():
    ...
    if len(trades) > 0:
        ...
        print(
            f"Assertividade   : {win_rate:.2f}% ({wins} Win / {len(trades) - wins} Loss)"
        )
    ...
    print(
        f"- Sinais Filtrados (Seguranca): {shadow.get('v22_candidates', 0) - len(trades)}"
    )
    ...
    if len(trades) < 3:
        print(
            "-> Baixa frequencia de trades: Considere reduzir o 'cooldown_minutes' no v22_locked_params.json."
        )
    if shadow.get("filtered_by_flux", 0) > 2:
        print(
            "-> O Filtro de Fluxo esta barrando entradas validas. Verifique a calibragem de 1.2x."
        )
