...
class DataCollector:
    ...
    async def start(self):
        ...
        while self.is_running:
            try:
                ...
                last_tick = (
                    self.bridge.get_market_data(self.symbol, n_candles=1)
                    .iloc[-1]
                    .to_dict()
                )
    ...
    async def flush_data(self):
        ...
        try:
            ...
            tick_filename = (
                f"{self.dataset_dir}/ticks_labeled_{self.symbol}_{timestamp_str}.csv"
            )
            df_labeled.to_csv(tick_filename, index=False)
            logging.info(
                f"💾 Dataset salvo e rotulado: {len(df_labeled)} ticks em {tick_filename}"
            )

        except Exception as e:
            logging.error(f"Erro ao rotular dados: {e}. Salvando bruto.")
            tick_filename = (
                f"{self.dataset_dir}/ticks_raw_{self.symbol}_{timestamp_str}.csv"
            )
