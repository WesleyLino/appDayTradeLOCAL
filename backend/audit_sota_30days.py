        ...
        for i in range(lookback, len(data)):
            ...
            if v22_buy or v22_sell:
                ...
                if time_ok and limit_ok and vol_ok and ai_ok:
                    ...
                    reason = ""
                    if not time_ok: reason += "Time "
                    if not limit_ok: reason += "Limit "
                    if not vol_ok: reason += "Vol "
                    if not ai_ok: reason += "AI_Score "
...
async def analyze_full_audit():
    ...
    if not df_trades.empty:
        ...
        print(f"Total Perdas: R$ {df_trades[df_trades['pnl_fin'] < 0]['pnl_fin'].sum():.2f}")
