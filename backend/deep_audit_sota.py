...
async def run_deep_audit():
    ...
    print(
        f"Lucro Líquido:    R$ {profit:.2f} ({(profit / config['initial_balance']) * 100:.2f}%)"
    )
    ...
    if shadow["tiers"]["80-85"] > 0:
        print(
            f"  1. Otimização de Score: Existem {shadow['tiers']['80-85']} sinais 'Quase Sniper'."
        )
        print(
            "     Ajustar o Trailing Stop para travar BE mais cedo pode permitir operar no Tier 80%."
        )
    else:
        print(
            "  1. Filtro Mestre: O Threshold de 85% está agindo perfeitamente contra ruído."
        )

    print(
        f"  2. Alpha Scaling: Com Win Rate de {win_rate}%, o escalonamento de lotes após 2 vitórias é seguro."
    )
    print(
        "  3. Sentiment Veto: Continue usando o filtro de sentimento para evitar reversões macro."
    )
