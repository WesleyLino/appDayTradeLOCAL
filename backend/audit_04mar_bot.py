...
async def run_mt5_analysis():
    ...
    if data.empty:
        print(f'❌ Nenhum dado encontrado para o dia de hoje ({target_date}). Verifique o pregão.')
        return

    print(f'✅ Dados carregados: {len(data)} candles capturados (Timeline: {data.index[0].strftime("%H:%M")} as {data.index[-1].strftime("%H:%M")}).')
    print('🧠 Executando simulação de redes complexas e análise de sinal... Isso pode demorar alguns segundos.')
    ...
    if len(trades) > 0:
        ...
        print(f'🟢 COMPRA: {len(buy_trades)} trades | PnL: R$ {buy_pnl:+.2f} | Assertividade: {buy_wr:.1f}%')
        print(f'🔴 VENDA:  {len(sell_trades)} trades | PnL: R$ {sell_pnl:+.2f} | Assertividade: {sell_wr:.1f}%')
        
    print('\n--- SHADOW ANALYSIS (Oportunidades Perdidas) ---')
    print(f'- Total de Sinais Brutos da IA (v22_candidates): {shadow.get("v22_candidates", 0)}')
    print(f'- Veto por Padrão de Incerteza (IA):             {shadow.get("filtered_by_ai", 0)}')
    print(f'- Veto por Microestrutura de Fluxo (CVD/OFI):    {shadow.get("filtered_by_flux", 0)}')
    print(f'- Falhas nos Componentes Restritivos:            {shadow.get("component_fail", {})}')
    ...
    for thresh in [0.65, 0.70, 0.75, 0.80]:
        for flux in [True, False]:
            tmp_tester = BacktestPro(symbol=symbol, initial_balance=capital, use_ai_core=True)
            ...
            t = tmp_tester.trades
            w = (len([x for x in t if x["pnl_fin"] > 0]) / len(t)) * 100 if len(t) > 0 else 0
    ...
    if best_config:
        ...
        print('Nenhuma combinação de re-treinamento rápido superou a estabilidade e previsibilidade de lucro do Golden State atual.')
