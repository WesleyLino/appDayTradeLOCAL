...
class DeepAnalyzer(BacktestPro):
    async def run_with_diagnostics(self):
        ...
        data['upper_bb'] = data['sma_20'] + self.opt_params['bb_dev'] * data['close'].rolling(window=20).std()
        data['lower_bb'] = data['sma_20'] - self.opt_params['bb_dev'] * data['close'].rolling(window=20).std()
        ...
        for i in range(lookback, len(data)):
            ...
            if not self.position:
                ...
                if technical_signal:
                    ...
                    if not time_ok: rejection_reason = "FORA DA JANELA"
                    elif not limit_ok: rejection_reason = "LIMITE DIARIO EXCEDIDO"
                    elif not cooldown_ok: rejection_reason = "COOLDOWN"
...
async def analyze():
    ...
    analyzer = DeepAnalyzer(symbol="WIN$", n_candles=600, initial_balance=1000.0, use_trailing_stop=True, **params)
    logs = await analyzer.run_with_diagnostics()
    
    print("\n--- ANALISE PROFUNDA DE SINAIS (FILTROS) ---")
    for log in logs:
        print(f"[{log['time']}] {log['event']} | {log['side'].upper()} | RSI: {log['rsi']:.1f} | Vol: {log['vol_mult']:.1f}x")
