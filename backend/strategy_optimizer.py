...
def traceiar_dia_com_filtros(
    ...
    bb_d       = cfg.bb_dev_override if cfg.bb_dev_override is not None else float(p.get('bb_dev', 2.0))
    vm         = cfg.vol_mult_override if cfg.vol_mult_override is not None else float(p.get('vol_spike_mult', 1.0))
    sl         = cfg.sl_override   if cfg.sl_override   is not None else float(p.get('sl_dist', 200.0))
    tp         = cfg.tp_override   if cfg.tp_override   is not None else float(p.get('tp_dist', 600.0))
    rsi_buy    = int(p.get('rsi_buy_level', 32)) if cfg.nome != "OPT4_CONSERVATIVE_RSI" else 20
    rsi_sell   = int(p.get('rsi_sell_level', 68)) if cfg.nome != "OPT4_CONSERVATIVE_RSI" else 80
    ...
    hl_abertura  = float((df_ab['high'] - df_ab['low']).mean()) if not df_ab.empty else 0.0
    ...
    trades_dia    = 0
    pnl_acumulado = 0.0
    limite_trades = cfg.limit_trades_override if cfg.limit_trades_override is not None else LIMIT_TRADES_DIA
    ...
    for i in range(max(25, rsi_p * 2), len(df)):
        ...
        if cfg.meta_diaria and pnl_acumulado >= cfg.meta_diaria:
            dia_pausado = "META_ATINGIDA"; continue
        if cfg.stop_diario and pnl_acumulado <= cfg.stop_diario:
            dia_pausado = "STOP_DIARIO"; continue
        ...
        r_sell = cfg.rsi_sell_override if cfg.rsi_sell_override is not None else rsi_sell

        cond_buy  = rsi_val <= r_buy  and float(row['close']) <= float(row['lower_bb']) and vol_ok and atr_ok
        cond_sell = rsi_val >= r_sell and float(row['close']) >= float(row['upper_bb']) and vol_ok and atr_ok
        ...
        if cfg.filtro_bias_h1 and motivo_veto is None:
            ...
            if lado == "BUY"  and not tendencia_alta: motivo_veto = "BIAS_H1_CONTRA"
            if lado == "SELL" and tendencia_alta:     motivo_veto = "BIAS_H1_CONTRA"
        ...
        if cfg.filtro_cooldown_stop and motivo_veto is None:
            ...
            if lado == "BUY"  and cooldown_buy  > ts: motivo_veto = "COOLDOWN_BUY"
            if lado == "SELL" and cooldown_sell > ts:  motivo_veto = "COOLDOWN_SELL"
            ...
            for j in range(i + 1, min(i + 120, len(df))):
                ...
                if lado == "BUY":
                    if float(f['low']) <= sl_val:
                        resultado = "STOP" if sl_val <= (entry - sl_atual + 5) else "TAKE_TRAIL"
                        pnl_pts = sl_val - entry
                        saiu_hora = f.name.strftime("%H:%M"); break
                    if float(f['high']) >= tp_val:
                        resultado = "TAKE"; pnl_pts = tp_atual; saiu_hora = f.name.strftime("%H:%M"); break
                    
                    if cfg.use_trailing:
                        dist = float(f['high']) - entry
                        if dist >= 200: sl_val = max(sl_val, entry + 10) # BE+10
                        if dist >= 400: sl_val = max(sl_val, float(f['high']) - 150) # Trail 150
                else: # SELL
                    ...
                    if cfg.use_trailing:
                        ...
                        if dist >= 200: sl_val = min(sl_val, entry - 10) # BE+10
                        if dist >= 400: sl_val = min(sl_val, float(f['low']) + 150) # Trail 150

            # Atualiza cooldown apos STOP
            if resultado == "STOP" and cfg.filtro_cooldown_stop:
                ts   = row.name.to_pydatetime().replace(tzinfo=None)
                dead = ts + timedelta(minutes=COOLDOWN_MIN)
                if lado == "BUY":  cooldown_buy  = dead
                else:              cooldown_sell = dead

        pnl_brl = round(abs(pnl_pts) * PONTO_BRL * cfg.lot_multiplier * (1 if pnl_pts > 0 else -1), 2)
        pnl_acumulado += pnl_brl

        sinais.append({
            "hora": hora, "hora_saida": saiu_hora, "lado": lado,
            "rsi": round(rsi_val, 1), "resultado": resultado,
            "pnl_pts": round(float(pnl_pts), 1), "pnl_brl": float(pnl_brl),
    ...
    def resumir(lado_str):
        ...
        n_atv = len(atv)
        return {
            "sinais_brutos": len(sub), "ativos": n_atv,
            "vetados": len(vet), "takes": len(takes), "stops": len(stops),
    ...
    return {
        "data": data_str, "dia_pausado": bool(dia_pausado),
        "hl_abertura": round(hl_abertura, 1),
        "buy": resumir("BUY"), "sell": resumir("SELL"),
        "pnl_dia": round(sum(s['pnl_brl'] for s in sinais if s['resultado'] != "VETADO"), 2),
...
async def executar_iteracao(df_all, p, cfg: FilterConfig) -> dict:
    ...
    ganho_total = sum(r['buy']['ganho_brl'] + r['sell']['ganho_brl'] for r in resultados)
    perda_total = sum(r['buy']['perda_brl'] + r['sell']['perda_brl'] for r in resultados)
    ...
    return {
        ...
        "buy_ativos":  ab,  "buy_takes":  tb,  "buy_stops":  sb,  "buy_assert":  buy_assert,
        "sell_ativos": as_, "sell_takes": ts,  "sell_stops": ss,  "sell_assert": sell_assert,
...
async def main():
    ...
    if df_all is None or df_all.empty:
        print("❌  Falha na coleta MT5."); return

    datas_disp = set(df_all.index.strftime('%Y-%m-%d').unique())
    print(f"✅   {len(df_all)} candles | {df_all.index[0].date()} → {df_all.index[-1].date()}")
    print(SEP)
    ...
    for cfg in ITERACOES:
        ...
        flag = "⭐" if (melhor is None or r['pnl_total_brl'] > melhor['pnl_total_brl']) else "  "
        if melhor is None or r['pnl_total_brl'] > melhor['pnl_total_brl']:
            melhor = r

        print(f"  {flag} {cfg.nome:<20} {r['total_assert']:>7.1f}% "
    ...
    for r in todos_resultados:
        ...
        print(f"  {r['config']:<22} {r['total_ativos']:>13} {total_t:>6} {total_s:>6} "
    ...
    print(f"    PnL Total: R$ {melhor['pnl_total_brl']:+.2f} | Assertividade: {melhor['total_assert']:.1f}% | PF: {melhor['profit_factor']:.2f}x")
    print(f"    BUY Assert: {melhor['buy_assert']:.1f}% | SELL Assert: {melhor['sell_assert']:.1f}%")
