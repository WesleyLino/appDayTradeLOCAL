...
async def run_1day_micro_test():
    ...
    if len(backtester.trades) > 0:
        ...
        print(
            f"Vitórias: {len(win_trades)} | Derrotas: {len(loss_trades)} | BE: {len(be_trades)}"
        )
    ...
    for t in backtester.trades:
        print(
            f"Saída: {t['exit_time'].strftime('%H:%M:%S')} | Side: {t['side']} | Pts: {t['pnl_points']:.1f} | R$: {t['pnl_fin']:.2f} | Motivo: {t['reason']}"
        )
