...
CAPITAL = 3000.0
DATAS   = [
    (2026, 2, 20), (2026, 2, 23), (2026, 2, 24),
    (2026, 2, 25), (2026, 2, 26), (2026, 2, 27),
]
THRESHOLDS = {
    "85% (ATUAL)":  0.85,
    "82% (PROPOSTO)": 0.82,
...
async def main():
    ...
    print(f"  Símbolo: {SYMBOL} | Capital: R$ {CAPITAL:.2f} | Período: 20/02–27/02/2026")
    print("=" * 72)
    ...
    for label in THRESHOLDS:
        r = resumo[label]
        print()
        print(f"  ▸ Configuração: {label}  (confidence_threshold={THRESHOLDS[label]:.2f})")
        print("  " + "-" * 62)
        for linha in detalhe[label]:
            print(linha)
        print("  " + "-" * 62)
        total_wr = (r["wins"] / r["trades"] * 100) if r["trades"] else 0
        print(f"  TOTAL  | {r['trades']:>3} trades | {total_wr:>5.1f}% WR | PnL: R$ {r['pnl']:>+8.2f}")
    ...
    delta_wr     = (
        (r2["wins"] / r2["trades"] * 100 if r2["trades"] else 0)
    ...
    print(f"  {labels[0]:30s}  Trades: {r1['trades']:>3} | WR: {(r1['wins']/r1['trades']*100 if r1['trades'] else 0):>5.1f}% | PnL: R$ {r1['pnl']:>+8.2f}")
    print(f"  {labels[1]:30s}  Trades: {r2['trades']:>3} | WR: {(r2['wins']/r2['trades']*100 if r2['trades'] else 0):>5.1f}% | PnL: R$ {r2['pnl']:>+8.2f}")
    print()
    ...
    if delta_pnl > 0 and delta_wr >= -3:
        print("  ✅ CONCLUSÃO: Threshold 82% SUPERIOR — mais trades C/ PnL positivo e WR aceitável")
        ...
        print("  🔒 CONCLUSÃO: Threshold 85% (ATUAL) é SUPERIOR — manter configuração atual.")
    print("=" * 72)
