...
async def run_calibration():
    ...
    for i, p in enumerate(combinations):
        ...
        if report_trades_len > 0:
            if report["total_pnl"] > best_pnl or (
                report["win_rate"] > best_winrate
                and report["total_pnl"] >= best_pnl * 0.90
            ):
                ...
                best_report = report
                print(
                    f"✨ Nova Combinação Superior Encontrada: Lucro R$ {best_pnl:.2f} | WR {best_winrate:.2f}% | Configs: {p}"
                )
    ...
    print(
        f"Lucro: R$ {baseline_report['total_pnl']:.2f} | WinRate: {baseline_report['win_rate']:.2f}% | Trades: {baseline_trades_len}"
    )
    print(
        f"DrawnDown: {baseline_report['max_drawdown']:.2f}% | Profit Factor: {baseline_report['profit_factor']:.2f}"
    )
    ...
    if best_params:
        ...
        print(
            f"Lucro: R$ {best_report['total_pnl']:.2f} | WinRate: {best_report['win_rate']:.2f}% | Trades: {best_trades_len}"
        )
        print(
            f"DrawnDown: {best_report['max_drawdown']:.2f}% | Profit Factor: {best_report['profit_factor']:.2f}"
        )
        print(f"Novos Ajustes: {best_params}")
    else:
        print("\n--- CONCLUSÃO ---")
        print(
            "Os parâmetros atuais já são os mais lucrativos e assertivos para o dia de hoje. Nenhuma melhoria absoluta foi validada."
        )
