...
def debug_indicators():
    ...
    files = [f for f in os.listdir('.') if f.startswith('temp_multi_') and f.endswith('.csv')]
    if not files:
    ...
    cols = ['close', 'rsi', 'sma_20', 'upper_bb', 'lower_bb', 'atr_current', 'vol_sma', 'tick_volume']
    ...
    bt.data['v22_buy_raw'] = bt.data['cond_rsi_buy'] & bt.data['cond_bb_buy'] & bt.data['cond_vol_buy']
    
    hits = bt.data[bt.data['v22_buy_raw'] == True]
    ...
    if len(hits) > 0:
        ...
        print(f"RSI: Min={bt.data['rsi'].min():.2f}, Max={bt.data['rsi'].max():.2f}, Avg={bt.data['rsi'].mean():.2f}")
        print(f"Distancia Close-BB_Lower: min={(bt.data['close'] - bt.data['lower_bb']).min():.2f}")
        print(f"Volume Ratio: max={(bt.data['tick_volume'] / bt.data['vol_sma']).max():.2f}")
