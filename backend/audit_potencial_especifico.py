...
async def run_audit_sota():
    ...
    bt = BacktestPro(symbol="WIN$", n_candles=15000, initial_balance=3000.0, use_ai_core=True)
    ...
    if not df_trades.empty:
        ...
        wr_buys = (len(buys[buys['pnl_fin'] > 0]) / len(buys) * 100) if not buys.empty else 0
        wr_sells = (len(sells[sells['pnl_fin'] > 0]) / len(sells) * 100) if not sells.empty else 0
