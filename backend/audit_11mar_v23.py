...
async def run_audit():
    try:
        ...
        if not bridge.connect():
            logger.error(
                "❌ Erro ao conectar ao MT5. Certifique-se que o terminal está aberto."
            )
            return
        ...
        if df.empty:
            logger.error(
                "❌ Nenhum dado encontrado para o dia 11/03. Verifique o histórico do MT5."
            )
            return
        ...
        if report is None:
            logger.error(
                "❌ O backtest retornou 'None'. Algo falhou internamente no bt.run()"
            )
            return
        ...
        trades_11mar = [
            t
            for t in trades_total
            if t["entry_time"].date() == datetime(2026, 3, 11).date()
        ]
        ...
        if not trades_df.empty:
            ...
            print(
                f"Operações COMPRA:  {len(buys):<3} | PnL Somado: R$ {buys['pnl_fin'].sum():.2f}"
            )
            print(
                f"Operações VENDA:   {len(sells):<3} | PnL Somado: R$ {sells['pnl_fin'].sum():.2f}"
            )
        else:
        ...
        if shadow.get("filtered_by_ai", 0) > 5:
            print(
                "1. Relaxar threshold de confiança em 10% para regimes de tendência comprovada."
            )
        if len(trades_11mar) == 0:
            print(
                "1. Reduzir exigência de 'Bollinger Squeeze' para 1.0 em dias de expansão de volatilidade."
            )
        print(
            "2. Calibrar Trailing Stop para travar 30% mais rápido em scalpings laterais."
        )
        print("=" * 80 + "\n")
