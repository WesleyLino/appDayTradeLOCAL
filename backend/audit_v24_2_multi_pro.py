...
async def run_multi_day_audit():
    ...
    for date_str in target_dates:
        ...
        bt = BacktestPro(
            symbol=symbol, n_candles=len(df), initial_balance=capital_inicial
        )
        bt.data = df
        ...
        logging.info(
            f"✅ FINALIZADO: PnL: R$ {pnl:.2f} | Trades: {trades_count} | Win Rate: {win_rate:.1f}%"
        )

        results.append(
            {"data": date_str, "pnl": pnl, "trades": trades_count, "win_rate": win_rate}
        )
        total_pnl += pnl
