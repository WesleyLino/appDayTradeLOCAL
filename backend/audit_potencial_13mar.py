...
async def analyze_potencial_13mar():
    ...
    logger.info(f"🚫 Vetados por Fluxo/Bias: {shadow.get('filtered_by_flux', 0) + shadow.get('filtered_by_bias', 0)}")
    ...
    logger.info(f"📉 VENDAS: {len(sells)} | Ganhos: {len([t for t in sells if t['pnl_fin'] > 0])}")
    
    # 7. Diagnóstico e Melhoria
    logger.info("\n💡 [DIAGNÓSTICO FINAL]")
    if len(trades_today) < 1 and shadow.get('v22_candidates', 0) > 5:
        logger.info("A IA viu as oportunidades, mas os filtros de segurança (confidence, rigor) os vetaram.")
        logger.info("Sugestão: Calibrar 'confidence_relax_factor' para 1.15.")
    elif len(trades_today) > 0 and (pnl_total < 0):
        logger.info("Trades realizados entraram em stop. Verifique Trailing Stop ou Distância de SL.")
    else:
        logger.info("Sistema operando com máxima assertividade para o capital de R$ 3k.")
