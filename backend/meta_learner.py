...
class MetaLearner:
    ...
    def load_model(self):
        ...
        if os.path.exists(self.model_path):
            try:
                ...
                logging.warning("Bibliotexa xgboost não encontrada. Meta-Learner desativado.")
            except Exception as e:
                logging.error(f"Erro ao carregar Meta-Learner: {e}")
        else:
            logging.info("Nenhum modelo Meta-Learner encontrado. Operando em modo coleta de dados.")
    ...
    def log_training_data(self, features, label=None, file_path="data/training/triple_barrier_data.csv"):
        ...
        """
        try:
            df = pd.DataFrame([features], columns=['ATR', 'OBI', 'Sentiment', 'Volatility', 'Hour', 'AI_Score'])
