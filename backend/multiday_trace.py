...
def traceiar_dia(df_all: pd.DataFrame, data_str: str, p: dict) -> dict | None:
    ...
    hl_abertura = float((df_abert['high'] - df_abert['low']).mean()) if not df_abert.empty else 0.0
    ...
    for i in range(max(20, rsi_p * 2), len(df)):
        ...
        cond_buy  = row['rsi'] <= rsi_buy  and row['close'] <= row['lower_bb'] and vol_ok and atr_ok
        cond_sell = row['rsi'] >= rsi_sell and row['close'] >= row['upper_bb'] and vol_ok and atr_ok
        ...
        a_favor    = (lado == "buy" and bias_h1 == "alta") or (lado == "sell" and bias_h1 == "baixa")
        ...
        for j in range(i + 1, min(i + 120, len(df))):
            f = df.iloc[j]
            if lado == "buy":
                if f['low']  <= sl_val: resultado = "STOP"; pnl_pts = -sl; saiu_hora = f.name.strftime("%H:%M"); break
                if f['high'] >= tp_val: resultado = "TAKE"; pnl_pts =  tp; saiu_hora = f.name.strftime("%H:%M"); break
            else:
                if f['high'] >= sl_val: resultado = "STOP"; pnl_pts = -sl; saiu_hora = f.name.strftime("%H:%M"); break
                if f['low']  <= tp_val: resultado = "TAKE"; pnl_pts =  tp; saiu_hora = f.name.strftime("%H:%M"); break

        pnl_financeiro = pnl_brl(abs(pnl_pts)) * (1 if pnl_pts > 0 else (-1 if pnl_pts < 0 else 0))
...
async def main():
    ...
    print(f"⚙️  Params: RSI={p['rsi_period']} | RSI-Buy<={p['rsi_buy_level']} / RSI-Sell>={p['rsi_sell_level']}")
    print(f"          SL={p['sl_dist']} | TP={p['tp_dist']} | BB-Dev={p['bb_dev']} | VolMult={p['vol_spike_mult']}")
    ...
    print(f"{'SELL':<8} {sell_total:>7} {total_sell_t:>7} {total_sell_s:>7} {sell_assert:>8.1f}% "
          f"R${ganho_pot_sell:>8.2f} R${perda_pot_sell:>8.2f} R${pnl_sell:>+8.2f}")
    print(f"{'-'*83}")
    print(f"{'TOTAL':<8} {buy_total+sell_total:>7} {total_buy_t+total_sell_t:>7} "
    ...
    if dias_pausados:
        ...
        for r in dias_pausados:
            print(f"    {r['data']}: HL abertura={r['hl_abertura']} pts | PnL se sem filtro: R$ {r['pnl_dia']:+.2f}")
    ...
    for i, r in enumerate(ranking):
        ...
        print(f"  {i+1:>2}. {r['data']} {flag} R${r['pnl_dia']:>+8.2f} "
