...
def export_to_onnx(
    ...
    if os.path.exists(model_path):
        ...
        logger.warning(
            f"Pesos não encontrados em {model_path}. Exportando modelo não treinado (apenas estrutura)."
        )
    ...
    try:
        ...
        try:
            ...
            ort_inputs = {
                ort_session.get_inputs()[0].name: dummy_input.numpy().astype(np.float16)
            }
            ort_outs = ort_session.run(None, ort_inputs)
            logger.info(
                f"Teste de Inferência ONNX (FP16): OK. Shape: {ort_outs[0].shape}"
            )
        except Exception as e_inf:
            logger.warning(
                f"Teste de inferência local falhou (Normal se CPU não suportar FP16 nativo): {e_inf}"
            )
