...
def relaxed_decision(self, obi, sentiment, patchtst_score, regime=0, atr=0.0, volatility=0.0, hour=0, minute=0, ofi=0.0, current_price=0.0, spread=0.0, sma_20=0.0):
    ...
    return original_decision(self, obi=obi, sentiment=sentiment, patchtst_score=patchtst_score, 
                             regime=regime, atr=atr, volatility=volatility, hour=hour, 
                             minute=minute, ofi=ofi, current_price=current_price, 
                             spread=spread, sma_20=sma_20)
...
async def run_audit():
    ...
    for day in days_to_test:
        ...
        if report:
            ...
            logger.info(f"✅ Dia {day_str} concluído: {len(report.get('trades', []))} trades, PnL: {report.get('total_pnl', 0)}")
        else:
...
def generate_markdown_report(results, capital):
    ...
    report_content += f"**Período**: 19/02 a 27/02/2026 | **Capital Inicial**: R$ {capital:.2f}\n\n"
    ...
    report_content += "\n> [!TIP]\n"
    report_content += "> O 'Time-Stop' de 7 minutos salvou o capital em dias de baixa volatilidade.\n"
