...
async def run_audit():
    ...
    if df_filtered.empty:
        ...
        print("⚠️ Dados filtrados vazios. Tentando localizar o dia no histórico total...")
    ...
    print("-" * 30)
    print(f"Status do Dia: {'🔴 PAUSADO POR VOLATILIDADE EXTREMA' if bt._dia_pausado_atr else '🟢 OPERACIONAL'}")
    ...
    if not df_p.empty:
        ...
        print(f"  - COMPRA: {len(buy_p)} trades | PnL Estimado: R$ {buy_p['pnl_fin'].sum():.2f}")
        print(f"  - VENDA:  {len(sell_p)} trades | PnL Estimado: R$ {sell_p['pnl_fin'].sum():.2f}")
        print(f"  - PnL BRUTO TOTAL: R$ {potential_total_pnl:.2f}")
    else:
        print("  - Mesmo ignorando a pausa, os filtros de IA (Score < 0.65) impediram entradas.")

    # 5. GERAR RELATÓRIO FINAL
    print("\n" + "💡 CONLUSÃO TÉCNICA")
    print("-" * 30)
    if potential_total_pnl < 0:
        print(f"✅ PROTEÇÃO EFICAZ: Ignorar a trava resultaria em um prejuízo de R$ {abs(potential_total_pnl):.2f}.")
    else:
        print(f"📈 POTENCIAL SACRIFICADO: A trava evitou um ganho de R$ {potential_total_pnl:.2f} em busca de segurança.")
    ...
    with open("backend/audit_06mar_results.md", "w", encoding="utf-8") as f:
        ...
        if not df_p.empty:
            ...
            f.write("- Mesmo sem a trava de volatilidade, os filtros de IA mantiveram a segurança.\n")
