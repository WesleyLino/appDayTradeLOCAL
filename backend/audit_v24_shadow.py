...
async def run_shadow_audit_11mar():
    logger.info(
        "💎 [v24 SHADOW AUDIT] Analisando 11/03 em busca de Oportunidades Perdidas"
    )
    ...
    for t in trades:
        logger.info(
            f"   - Trade {t['side']} às {t['entry_time']} | PnL: R$ {t['pnl_fin']:.2f} | Motivo: {t['reason']}"
        )

    if not trades:
        logger.warning("⚠️ O motor V24 foi EXTREMAMENTE conservador em 11/03.")
        logger.info(
            "💡 Motivo provável: Alta volatilidade ou falta de sincronismo de volume no histórico estático."
        )
