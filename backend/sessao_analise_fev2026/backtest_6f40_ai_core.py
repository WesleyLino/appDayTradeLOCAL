...
import torch # Import necess??rio para InferenceEngine
import time
import gc
...
class AICore:
    def __init__(self):
        ...
        self.ema_alpha = 0.2 # Fator de suaviza????o
        ...
        # [SOTA v4] NOVAS CONFIGURA????ES DE ASSERTIVIDADE
        self.h1_trend = 0 # -1 (Baixa), 0 (Neutro), 1 (Alta)
        self.prob_lot_tiers = {
            "low": 0.25,      # Confian??a 70-75%
            "medium": 0.75,   # Confian??a 75-85%
            "high": 1.0       # Confian??a > 85%
        }
        
        # [SOTA v25] SENTIMENTO MACRO (EMA 60m)
        self.sentiment_ema = 0.0
        self.sentiment_alpha = 0.033 # ~60 per??odos para estabiliza????o (1-min)
        self.macro_bull_lock = False
        self.macro_bear_lock = False

        self.sentiment_anchor_price = 0.0
        self.sentiment_anchor_time = 0
        self.max_sentiment_drift = 150.0 # Pontos do WIN para absor????o total
        ...
        # [ALPHA v1.0] CONFIGURA????ES DE FLEXIBILIZA????O
    ...
    def fetch_latest_news(self):
        """
        Coleta not??cias de 'backend/news_feed.txt' para an??lise de sentimento.
        Permite que o usu??rio insira not??cias em tempo real salvando no arquivo.
        """
        ...
        return "Mercado aguardando novos dados. Sem not??cias relevantes no momento."

    # [Fase 27] L??gica evaluate_opportunity removida. 
    # O Bot agora decide usando calculate_decision diretamente para maior controle de par??metros.

    # [SOTA v5] Property para Sincroniza????o Autom??tica de ??ncora
    @property
    ...
    def latest_sentiment_score(self, value):
        # [SOTA v25] C??LCULO DE EMA MACRO (Suaviza????o de 1h) - Funciona em Live e Backtest
        self.sentiment_ema = (value * self.sentiment_alpha) + (self.sentiment_ema * (1 - self.sentiment_alpha))
        
        # Atualiza????o das Travas Macro
        self.macro_bull_lock = self.sentiment_ema > 0.15
        self.macro_bear_lock = self.sentiment_ema < -0.15

        # Reset de ??ncora se o novo score for significativamente diferente (> 0.3)
        # Isso centraliza a l??gica para Live, Sniper e Backtest.
    ...
    async def update_sentiment(self):
        """
        L?? o score de sentimento do arquivo gerado pelo NewsSentimentWorker.
        Remove lat??ncia e custo de chamadas repetitivas ?? API no loop principal.
        """
        ...
        if not os.path.exists(sentiment_file):
            ...
            if "Sem not??cias relevantes" not in legacy_news:
                logging.debug("?????? Usando news_feed.txt como fallback (Worker Inativo)")
            return self.latest_sentiment_score

        try:
            if os.path.getsize(sentiment_file) == 0:
                logging.warning("?????? news_sentiment.json vazio.")
                return self.latest_sentiment_score
            ...
            # [FASE 28] TTL DIN??MICO POR N??VEL DE RISCO
            # Eventos sist??micos t??m vida longa; ru??do expira r??pido.
            ttl_map = {
                "EXTREME": 8 * 3600,  # 8h  ??? eventos geopol??ticos/sist??micos persistem
                "HIGH":    2 * 3600,  # 2h  ??? macro surpresa, normaliza no preg??o
                "MEDIUM":  30 * 60,   # 30min ??? not??cia corporativa, absor????o r??pida
                "LOW":     10 * 60,   # 10min ??? ru??do, descart??vel
            }
            ...
            if age_seconds > ttl:
                ...
                # O reset de ??ncora e EMA agora s??o feitos automaticamente pelo setter de latest_sentiment_score
                self.latest_sentiment_score = max(-1.0, min(1.0, score))
                
            return self.latest_sentiment_score
        except (json.JSONDecodeError, ValueError) as je:
            logging.error(f"??? Corrup????o de dados no JSON de sentimento: {je}")
            return self.latest_sentiment_score
        except Exception as e:
    ...
    def detect_spoofing(self, order_book, time_sales):
        """
        Detecta spoofing comparando volume do book com hist??rico recente.
        Implementa a regra de 'Toxic Flow': sumi??o de ordens grandes sem trade.
        """
        ...
        # Sugerido: N??vel 1: 1.0, N??vel 2: 0.6, N??vel 3: 0.3, N??vel 4: 0.1
        ...
        # 2. Detec????o de Toxic Flow (Remo????o S??bita vs Execu????o Real)
        if self.prev_book is not None:
            # Usamos o RAW aqui, para comparar com a agress??o real de mercado e medir lotes f??sicos cancelados
            ...
            # C??lculo exato de agress??o (Volume Executado por Lado)
            # time_sales deve ser filtrado se poss??vel, ou usar heur??stica
            # Assumindo que DataCollector entrega 'real_volume' e 'flags'
            # Se n??o tiver flags, usamos volume total como proxy conservador
            
            sell_aggression_vol = 0 # Agress??o de Venda (consome Bid)
            buy_aggression_vol = 0 # Agress??o de Compra (consome Ask)
            
            if time_sales is not None and hasattr(time_sales, 'empty') and not time_sales.empty:
                # Na B3, TICK_FLAG_BUY (agress??o compra) e TICK_FLAG_SELL (agress??o venda)
            ...
            # --- L??GICA ALPHA-X: CANCELLATION RATE ---
            ...
            # Cancellation Rate: Propor????o do volume removido que N??O foi trade
            ...
            # Thresholds Din??micos (HFT v2.1)
            threshold_bid = max(20, total_bid_raw * 0.1)
            threshold_ask = max(20, total_ask_raw * 0.1)
            
            # [SPOOFING DE COMPRA]: Lotes sumindo no Bid sem agress??o correspondente
            ...
            # [SPOOFING DE VENDA]: Lotes sumindo no Ask sem agress??o correspondente
        ...
        # Ajusta o OBI final com base no fluxo t??xico
        final_signal = self.obi_ema + self.toxic_flow_score
        
        # [AUDITORIA] Log de Press??o OBI Ponderada
        if abs(final_signal) > 0.4:
            logging.info(f"??????? [OBI-SOTA] Press??o (Ponderada): {final_signal:.2f} | Fluxo T??xico: {self.toxic_flow_score:.2f}")
    ...
    def detect_regime(self, volatility, obi):
        ...
        if len(self.regime_history) < 20: 
            return 0 # Default: Consolida????o
            
        if len(self.regime_history) > 100:
            self.regime_history.pop(0)
            
        # Fit peri??dico para n??o sobrecarregar o loop (ex: a cada 100 amostras)
        ...
        try:
            ...
            # 0: Baixa Vol (Consolida????o), 1: M??dia Vol (Tend??ncia), 2: Alta Vol (Ru??do)
    ...
    def calculate_probabilistic_lot(self, score, quantile_confidence):
        """
        [SOTA v4] Calcula o multiplicador de lote baseado na convic????o da IA.
        """
        # Se quantile_confidence for NORMAL, o rigor ?? maior
    ...
    def update_h1_trend(self, h1_data):
        ...
        # Heur??stica simples: Pre??o acima/abaixo da m??dia de 20 per??odos em H1
        ma20 = h1_data['close'].rolling(20).mean().iloc[-1] if len(h1_data) >= 20 else last_close
        
        if last_close > ma20 * 1.002: # 0.2% acima da m??dia
            self.h1_trend = 1 # Alta
        elif last_close < ma20 * 0.998: # 0.2% abaixo da m??dia
    ...
    def update_sentiment_anchor(self, price):
        """
        [SOTA v5] Define o pre??o ??ncora para o decaimento de sentimento.
    ...
    def calculate_decision(self, obi, sentiment, patchtst_score, regime=0, atr=0.0, volatility=0.0, hour=0, minute=0, ofi=0.0, current_price=0.0, spread=0.0, sma_20=0.0, wdo_aggression=0.0):
        """
        [SOTA v5] Fluxo de Decis??o com Camadas de Precis??o
        [SOTA v25.3] Adicionado sma_20 para Mean Reversion Guard.
        """
        # 0. [SOTA v5] Sentiment Decay Adaptativo (Absor????o por pre??o)
        if self.sentiment_anchor_price > 0 and abs(sentiment) > 0.1:
            ...
            if decay_factor < 0.2:
                logging.debug(f"???? SENTIMENTO ABSORVIDO: Pre??o moveu {price_drift:.1f} pts. Neutralizando.")
        # 1. Normaliza????o Inicial
        uncertainty_abs = 0.0
        forecast_ref = 1.0
        q10 = q50 = q90 = None
        ...
        # 3. EMA de OBI para Filtro de Diverg??ncia
        ...
        if regime == 2 or abs(ofi) > 2.0:
            uncertainty_threshold = 0.20 # Rigor em ru??do
            logging.info("??????? FILTRO DE RU??DO: Rigor 0.20")
        ...
        if is_opening_session and self.opening_rigor_enabled:
            ...
            logging.info(f"??????? RIGOR DE ABERTURA ATIVO: 09:{minute:02d}. Filtros restritos habilitados.")

        # [PRO] In??rcia de Volatilidade
        if atr > 0:
            self.atr_history.append(atr)
            if len(self.atr_history) > 10: self.atr_history.pop(0)
            if len(self.atr_history) >= 5:
                atr_inertia = atr - self.atr_history[-5]
                if atr_inertia < -12.0: # Exaust??o
                    uncertainty_threshold *= 0.85 
                    logging.info(f"???? EXAUST??O DE VOLATILIDADE: Apertando o rigor ({atr_inertia:.1f})")

        # 5. Verifica????o de Vetos (Fail-Safes)
        ...
        if ia_in_cooldown and self.cooldown_mode == "HARD":
            ...
            if quantile_confidence == "VERY_HIGH" and uncertainty_rel < self.uncertainty_high_conviction_cap:
                ...
                logging.warning(f"??????? ENTRADA ULTRA-CONSERVADORA: Alta incerteza ({uncertainty_rel*100:.1f}%)")
            else:
                veto_reason = f"ALTA_INCERTEZA_FAILSAFE ({uncertainty_rel*100:.1f}% > {uncertainty_threshold*100:.1f}%)"
        
        # [PRO] Filtro de Diverg??ncia de Fluxo Inter-Mercados (WDO vs WIN)
        if veto_reason is None and wdo_aggression != 0.0:
            ...
            if ai_dir > 0 and wdo_aggression > 1.5:  
                ...
                logging.info("??????? VETO INTER-MERCADOS: Compra vetada por fluxo massivo de alta no WDO.")
            elif ai_dir < 0 and wdo_aggression < -1.5:
                veto_reason = "WDO_CROSS_CORRELATION_VETO_SELL_BLOCKED"
                logging.info("??????? VETO INTER-MERCADOS: Venda vetada por fluxo massivo de baixa no WDO.")

        # [PRO] Filtro de Diverg??ncia Interna de Fluxo
        ...
        if veto_reason is None:
            ai_dir = np.sign(norm_patchtst)
            if self.h1_trend == 1 and ai_dir < 0: # Venda contra tend??ncia de alta H1
                veto_reason = "H1_BULLISH_TREND_VETO"
            elif self.h1_trend == -1 and ai_dir > 0: # Compra contra tend??ncia de baixa H1
        ...
        if veto_reason is None:
            ...
            if ai_dir_raw < 0:
                if self.macro_bull_lock:
                    ...
                    logging.info("??????? [ANTI-NOISE] Venda barrada pela tend??ncia de alta macro.")
            elif ai_dir_raw > 0:
                if self.macro_bear_lock:
                    veto_reason = f"MACRO_BEAR_BLOCK (EMA {self.sentiment_ema:.2f} < -0.15)"
                    logging.info("??????? [ANTI-NOISE] Compra barrada pela tend??ncia de baixa macro.")
        ...
        if veto_reason is None and sma_20 > 0 and atr > 0:
            ...
            if ai_dir_raw > 0 and dist_pts > (self.mean_reversion_threshold * atr):
                ...
                logging.info(f"??????? [ANTI-EXHAUSTION] Compra bloqueada: Pre??o esticado ({self.mean_reversion_threshold} ATR).")
                ...
                logging.info(f"??????? [ANTI-EXHAUSTION] Venda bloqueada: Pre??o esticado ({self.mean_reversion_threshold} ATR).")

        # [SOTA v5] Spread Veto (Prote????o contra Slippage)
        if veto_reason is None and spread > self.spread_veto_threshold:
            ...
            logging.warning(f"?????? VETO DE SPREAD: {spread:.1f} pts > {self.spread_veto_threshold}")

        # 6. C??lculo do Score Final
        if veto_reason:
            norm_patchtst = 0.0
        ...
        if self.meta_learner.model is not None:
            try:
                ...
                if 0.45 <= meta_prob <= 0.55 and veto_reason is None:
                    lot_multiplier *= self.lot_multiplier_partial
                    logging.info(f"???? REGRA DO SIL??NCIO: Meta-Learner neutro ({meta_prob:.2%})")
            except: pass
        ...
        # Dire????o - [SOTA v25.6] SWEET SPOT PUSH (85-90%+)
        buy_threshold = 90.0
        sell_threshold = 10.0
        ...
        # [SOTA v5] Dimensionamento de Lote Probabil??stico e Ajuste de Abertura
        if veto_reason is None and direction != "NEUTRAL":
            ...
            if is_opening_session and self.opening_rigor_enabled:
                lot_multiplier *= 0.5 # For??a exposi????o reduzida na abertura
                logging.info(f"??? LOTE REDUZIDO NA ABERTURA: {lot_multiplier:.2f}x")
            
            # [ALPHA] Redu????o din??mica em Cooldown
            if ia_in_cooldown and self.cooldown_mode == "DYNAMIC":
                lot_multiplier *= 0.25
                logging.warning("?????? [COOLDOWN DIN??MICO] Lote reduzido p/ 25% devido a perdas recentes.")
            
        # [SOTA v5] Take Profit Ajustado pelo Spread (Compensa????o)
        tp_adj = 1.0
        if spread > 1.0:
            tp_adj = 1.0 + (spread / 100.0) # Aumenta TP proporcionalmente ao custo
            logging.debug(f"???? TP AJUSTADO PELO SPREAD: +{spread:.1f} pts compensa????o ({tp_adj:.2f}x)")
    ...
    def get_directional_probability(self, market_data) -> float:
        """
        [INTELIG??NCIA DO SUCESSO] Retorna a probabilidade da IA para a continua????o direcional.
        Simula/calcula se a for??a institucional tem > 80% de chance de continua????o.
        Como o backtest OHLCV de 1M ?? r??pido, usaremos uma heur??stica robusta aqui
        se o motor de infer??ncia profundo for muito lento, ou o output do Meta-Learner.
        """
        # Em produ????o real (Tick a Tick), isso consulta o PatchTST/Conformal Prediction.
        ...
        try:
            # Pega os ??ltimos 3-5 candles para avaliar a in??rcia direcional
...
class InferenceEngine:
    """
    [AMD OPTIMIZED] Motor de infer??ncia h??brido (ONNX Runtime + PyTorch Fallback).
    Prioriza DirectML para GPUs AMD (RX 6650 XT) visando lat??ncia < 30ms.
    """
    ...
    def check_resources(self):
        """Verifica se os arquivos de modelo necess??rios existem."""
        missing = []
        if self.onnx_path and not os.path.exists(self.onnx_path):
             # Apenas reporta se n??o existir, mas load_model tem fallback
             pass 
        
        if self.model_path and not os.path.exists(self.model_path):
             missing.append(f"Model File ({self.model_path})")
             
        # Retorna lista vazia se pelo menos um modo puder ser carregado ou se n??o houver path configurado (inicio frio)
    ...
    def load_model(self):
        ...
        try:
            logging.info("?????? INFERENCE ENGINE: Iniciando fallback para PyTorch (CPU)...")
            ...
            if self.model_path and os.path.exists(self.model_path):
                ...
                self.model.eval()
                logging.info(f"?????? INFERENCE ENGINE: Rodando em PyTorch (CPU) | Channels: {c_in} | Layers: {n_layers}")
            else:
                logging.warning(f"Pesos n??o encontrados em {self.model_path}. Usando modelo n??o treinado ({c_in} channels).")
        except Exception as e:
        ...
        if self.target_onnx and os.path.exists(self.target_onnx):
            try:
                ...
                if current_mtime > self.last_mtime:
                    logging.info("?????? INFERENCE ENGINE: Novo modelo ONNX detectado! Recarregando...")
                    self.load_model()
            except Exception as e:
                logging.error(f"Erro ao verificar updates do modelo: {e}")

    def _warmup_onnx(self):
        """Executa uma infer??ncia dummy para alocar mem??ria na GPU."""
        try:
            # Tentar detectar c_in da sess??o ONNX
            ...
            logging.info(f"???? DirectML Warm-up Conclu??do (Channels: {c_in}).")
        except Exception as e:
            logging.warning(f"Erro no warm-up ONNX: {e}")

    async def predict(self, dataframe):
        """Interface p??blica n??o-bloqueante."""
    ...
    def _predict_sync(self, dataframe):
        """L??gica de infer??ncia unificada para m??ltiplos canais."""
        try:
            ...
            try:
                ...
                if all(col in dataframe.columns for col in required_cols):
                    ...
                    logging.debug(f"?????? Column mismatch. Columns present: {list(dataframe.columns)}")
                    df = dataframe.copy()
                    ...
                    # Heur??stica de Microestrutura (Fase 20)
            ...
            # [FASE 28] NORMALIZA????O DIN??MICA (Log-Returns por Janela)
            # Se os dados cont??m valores > 10 (pre??os brutos), aplicamos log-returns
            if np.max(np.abs(input_data[:, :4])) > 10.0:
                # Aplicamos diferencia????o apenas nas colunas de pre??o (OHLC)
                # As colunas de volume e indicadores (CVD/OFI) j?? s??o estacion??rias ou tratadas
            ...
            if np.isnan(input_tensor).any():
                logging.warning("?????? NaN detectado no input tensor. Retornando neutro.")
                return {
                    "score": 0.5, "forecast_norm": 0.0, "lower_bound_norm": 0.0,
                    "upper_bound_norm": 0.0, "uncertainty_norm": 1.0, "confidence": 0.0
                }
            ...
            if self.use_onnx:
                try:
                    ...
                    if self.model is not None:
                        ...
                        # Sem fallback dispon??vel: retorna neutro sem quebrar o loop
                        logging.warning("?????? Sem modelo ONNX nem PyTorch. Retornando score neutro.")
                        return {
                            "score": 0.5, "forecast_norm": 0.0, "lower_bound_norm": 0.0,
                            "upper_bound_norm": 0.0, "uncertainty_norm": 1.0, "confidence": 0.0
                        }
            ...
            # O output q50 ?? um Z-Score direcional, porque a RevIN normatiza o input mas n??o desnormaliza a sa??da.
            
            # Para o WIN$ M1, mapeamos o Z-score de -2.5 a +2.5 para o score de ~0.0 a 1.0.
            # No M1, a varia????o (z-score) tende a ser pequena (ex: 0.4, 0.6).
            ...
            # C??lculo de Incerteza (Spread da distribui????o Conformal)
