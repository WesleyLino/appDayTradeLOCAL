...
class BacktestPro:
    def __init__(self, symbol="WIN$", n_candles=5000, timeframe="M1", data_file=None, **kwargs):
        ...
        self.ai = AICore()
        # Tenta carregar o motor de infer??ncia (se existir o peso SOTA)
        try:
            self.inference = InferenceEngine(model_path="backend/patchtst_weights_sota.pth")
            self.ai.inference_engine = self.inference
            logging.info("???? Motor de Infer??ncia SOTA carregado.")
        except Exception as e_engine:
            self.inference = None 
            self.ai.inference_engine = None
            logging.warning(f"?????? Pesos SOTA n??o carregados ({e_engine}). Usando modo degenerado (Sentiment/OBI).")
            
        self.risk = RiskManager()
        self.collector = DataCollector(symbol)
        
        # Par??metros Edit??veis no Grid Search - [ANTIVIBE-CODING]
        # Carrega Golden Params se existirem, sen??o usa defaults validados em 23/02/2026
        locked_params = {}
        try:
            import json
            ...
            if os.path.exists(params_path):
                with open(params_path, "r") as f:
                    ...
                    logging.info("??????? Golden Params V22 carregados com sucesso.")
        except Exception as e:
            logging.warning(f"?????? Falha ao carregar Golden Params ({e}). Usando defaults hardcoded.")
        ...
        self.data = None # Para inspe????o externa

    async def load_data(self):
        if self.data_file is not None and os.path.exists(self.data_file):
            logging.info(f"???? Lendo DataFrame Est??tico em O(1) do filepath: {self.data_file} (Amostra: {self.n_candles} velas)")
    ...
    def simulate_oco(self, row, position):
        """
        Simula execu????o OCO e Trailing Stop SOTA (Alpha V22 Pro).
        """
        side = position['side']
        sl = position['sl']
        tp = position['tp']
        ...
        if side == 'buy':
            ...
            # 2. L??gica de Trailing Stop SOTA
            if row['low'] <= sl:
                return 'SL', sl
    ...
    async def run(self):
        ...
        logging.info(f"???? Iniciando Simula????o High Fidelity com {len(data)} velas...")
        ...
        data['rsi'] = data['rsi'].fillna(50.0) # Estabiliza in??cio
        ...
        for i in range(lookback, len(data)):
            ...
            # [SOTA v5] Sincronizar ??ncora de Sentimento (Price-Based Decay)
            self.ai.update_sentiment_anchor(row['close'])

            # 2. L??gica de Entrada (Se estiver zerado)
            if not self.position:
                # --- INDICADORES ALPHA V21 (Pr??-calculados) ---
                rsi = row['rsi']
                ...
                # [SOTA v24] Cooldown Din??mico por Convic????o
                ...
                if use_ai_core:
                    # Preparar m??tricas sincronizadas
                    ...
                    # Predi????o PatchTST (Agora com normaliza????o autom??tica no AICore)
                    patchtst_data = 0.0
                    if self.ai.inference_engine is not None:
                        # Extrair janela para predi????o
                    ...
                    # AI Decision (Passando OFI se dispon??vel)
                    # No backtest OHLCV, simulamos OFI como 0.5 * sentiment para manter consist??ncia
                    ...
                    if v22_buy or v22_sell:
                        logging.debug(f"???? Decis??o da IA: {ai_decision}")
                ...
                # --- [LEGACY] INTELIG??NCIA DO SUCESSO: REGIME MAESTRO & SCALPING ADAPTATION ---
                else:
                    ...
                    # Sinais V22 (Estrat??gia de Revers??o de Volatilidade - Mean Reversion)
                    cond_rsi_buy = (rsi < 30)
                ...
                # Isso permite ver no relat??rio de auditoria quantos sinais a IA vetou.
                ...
                # [AGRESSIVO] Limite de 60% de perda di??ria
                limit_loss = self.initial_balance * 0.60
                risk_ok = self.daily_pnl > -limit_loss
                
                # Ignora limite num??rico de trades no modo agressivo
                limit_ok = True 
                ...
                # Se estiver usando AI Core, respeitamos a decis??o e estabilidade real do modelo
                if not use_ai_core:
                    # No modo legado, simulamos a confian??a da IA com base no RSI e Volume (Proxy)
                ...
                if (v22_buy or v22_sell) and not (time_ok and risk_ok and limit_ok and vol_stable and cooldown_ok and ai_filter_ok and sentiment_ok and flux_ok):
                    logging.debug(f"?????? Signal BLOCKED at {row.name}: time={time_ok}, risk={risk_ok}, limit={limit_ok}, vol_stable={vol_stable}, cooldown={cooldown_ok}, ai={ai_filter_ok}, flux={flux_ok}, sent={sentiment_ok}")
                ...
                if (v22_buy or v22_sell) and time_ok and risk_ok and limit_ok and vol_stable and cooldown_ok and ai_filter_ok and sentiment_ok and flux_ok:
                    ...
                    # [ALPHA] L??gica de Execu????o Diferenciada
                    exec_strat = ai_decision.get('execution_strategy', 'PASSIVA') if use_ai_core else "PASSIVA"
                    entry_price = row['close']
                    
                    if exec_strat == "MERCADO":
                        # Simula slippage/spread na agress??o a mercado
                        entry_price = row['close'] + (current_spread if v22_buy else -current_spread)
                        comment = "ALPHA_MERCADO"
                    else:
                        comment = "ALPHA_SNIPER"
                        entry_price = row['close'] # Ordem limite no pre??o atual (simplificado)

                    # --- [DYNAMIC ATR TARGETS V3] ---
                    # Replica a l??gica do RiskManager ajustada por regime
                    tp_mult = 1.0 
                    sl_mult = 1.3

                    if current_regime == 1: # Trend
                        tp_mult = 1.5
                        # [SOTA v24] ALVO ADAPTATIVO: +20% TP se alinhado com Ultra Tend??ncia
                        tp_mult *= 1.2
                    elif current_regime == 0: # Lateral
                        tp_mult = 0.8
                    
                    # ATR atual j?? calculado no in??cio do loop: row['atr_current']
        ...
        logging.info("???? Simula????o conclu??da.")
        if self.position:
            # For??a o fechamento da ??ltima posi????o para fins de relat??rio final no backtest
    ...
    def _close_trade(self, price, reason, exit_time):
        ...
        # C??lculo financeiro (Simplificado: WIN=0.20/pt, WDO=10.00/pt)
        ...
        # Atualiza m??tricas de lote din??mico tracker
        if pnl_fin > 0:
    ...
    def generate_report(self):
        ...
        if df_trades.empty:
            logging.warning("Nenhum trade realizado no per??odo.")
            return
        ...
        # Gr??ficos com Plotly
        fig = make_subplots(rows=2, cols=1, subplot_titles=("Curva de Equity", "Distribui????o de PnL por Trade"))
        ...
        fig.update_layout(height=800, title_text=f"Relat??rio Backtest AlphaX - {self.symbol}")
        ...
        print(f"Lucro L??quido: R$ {total_pnl:.2f}")
