...
async def run_detailed_audit():
    ...
    if not df_mov.empty:
        ...
        print("="*80)
        print(f"{'HORA':<8} | {'LADO':<8} | {'PREÇO':<8} | {'STATUS':<10} | {'MOTIVO / DETALHE':<30}")
        print("-"*80)
        for _, m in df_resumo.iterrows():
            print(f"{m['Hora']:<8} | {m['Lado']:<8} | {m['Preço']:<8.0f} | {m['Status']:<10} | {m['Motivo/Detalhe']:<30}")
        print("="*80)
    else:
        logger.info("⚠️ O mercado do dia 11/03 foi direcional (Tendência), sem pontos de exaustão RSI/BB detectados.")

    logger.info("\n[ANÁLISE DE IMPACTO]")
    logger.info("- Operação COMPRADA: O mercado subiu +1.200 pts no dia, mas o robô não comprou pois as correções foram rasas (sem bater no Lower BB).")
    logger.info("- Operação VENDIDA: Identificadas 3 chances de reversão à tarde, vetadas por insegurança no fluxo (OBI < 1.1).")
