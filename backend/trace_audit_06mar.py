...
async def trace_all_candidates():
    ...
    bt = BacktestPro(
        symbol="WIN$", n_candles=3000, initial_balance=capital, **strategy_params
    )
    ...
    for i in range(20, len(df)):
        ...
        is_buy_signal = (row["rsi"] < 30 and row["close"] < row["lower_bb"]) and (
            row["tick_volume"] > row["vol_sma"] * v_mult
        )
        is_sell_signal = (row["rsi"] > 70 and row["close"] > row["upper_bb"]) and (
            row["tick_volume"] > row["vol_sma"] * v_mult
        )
