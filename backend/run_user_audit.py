...
async def run_user_audit():
    ...
    logging.info(
        "📥 Solicitando histórico massivo do MT5 (15.000 velas), por favor aguarde..."
    )
    ...
    for target_date in dates_to_test:
        ...
        if not any(day_mask):
            logging.warning(
                f"⚠️ Dia {date_str} sem negociação ou sem dados no histórico retornado."
            )
        ...
        tester.opt_params["use_flux_filter"] = (
            False  # Desativar para ver potencial IA puro primeiro
        )
        ...
        logging.info(
            f"   - {len(tester.data)} velas enviadas para o backtester do dia."
        )
        ...
        detailed_reports += (
            f"- Oportunidades Vetadas pela IA (< 85% de confiança): {missed_ai}\n"
        )
        detailed_reports += f"- Oportunidades Vetadas por Fluxo Fraco: {missed_flux}\n"
        detailed_reports += (
            f"- Saídas Antecipadas (Proteção Flash-Exit): {flash_exits}\n"
        )
    ...
    report_path = os.path.join(
        os.path.dirname(__file__), "relatorio_auditoria_usuario.md"
    )
