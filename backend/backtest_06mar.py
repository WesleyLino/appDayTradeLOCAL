...
async def run_audit():
    ...
    print(
        f"⚙️ Configuração Ativa: RSI={strategy_params.get('rsi_period')} | OCO={strategy_params.get('sl_dist')}/{strategy_params.get('tp_dist')}"
    )
    ...
    results = {
        ...
        "shadow_no_atr": {
            ...
            "buy_pnl": sum(
                t["pnl_fin"] for t in bt_shadow_no_atr.trades if t["side"] == "buy"
            ),
            "sell_pnl": sum(
                t["pnl_fin"] for t in bt_shadow_no_atr.trades if t["side"] == "sell"
            ),
        },
        "brute_force": {
    ...
    print(
        f"Brute Force: PnL R$ {results['brute_force']['pnl']:.2f} ({results['brute_force']['trades']} trades)"
    )
