...
async def run_audit():
    ...
    logging.info(f"🚀 INICIANDO AUDITORIA FEVEREIRO 2026 - QUANTUMTRADE SOTA ({symbol})")
    ...
    for target_date in dates_to_test:
        ...
        if len(day_events) == 0:
            logging.warning(f"⚠️ Sem dados para o dia {target_date.strftime('%d/%m/%Y')}.")
            continue
        ...
        day_report += f"- **Vetos por Fluxo/Volume**: {shadow.get('filtered_by_flux', 0)}\n"
        ...
        if day_pnl < 0:
            if shadow.get('filtered_by_ai', 0) > 5:
                melhorias.append("- **IA Conservadora**: Muitos sinais vetados em dia de perda. Considerar recalibração de incerteza.")
            if shadow.get('filtered_by_bias', 0) > 3:
                melhorias.append("- **Veto de Tendência [H]**: Protegeu de entradas contra a tendência, mas pode ter bloqueado correções lucrativas.")
        else:
            melhorias.append("- **Calibragem Gold V22**: Mantida estabilidade e proteção de capital.")
            
        day_report += "### 🛠️ Sugestões de Melhoria\n"
        day_report += "\n".join(melhorias) if melhorias else "- Sem sugestões críticas para este cenário."
        day_report += "\n\n---\n"
