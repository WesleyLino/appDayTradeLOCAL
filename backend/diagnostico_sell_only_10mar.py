...
async def diagnosticar_h1_10mar():
    ...
    console.info(
        f"   Abertura: {day_data.index[0].strftime('%H:%M')} | "
    ...
    console.info(
        f"   Variação Dia:    {day_data['close'].iloc[-1] - day_data['close'].iloc[0]:+.0f} pts"
    )
    console.info("")
    ...
    console.info("-" * 70)
    console.info(
        f"{'Horário':<10} {'Preço':>8} {'MA60 M1':>10} {'Δ%':>8} {'H1 Trend':>12} {'Sell-Only?':>12}"
    )
    console.info("-" * 70)
    ...
    for j, (ts, row) in enumerate(day_data.iterrows()):
        ...
        if trend != estado_anterior or ts.minute == 0:
            trend_str = (
                "⬆️  ALTA (+1)"
                if trend == 1
                else ("⬇️  BAIXA (-1)" if trend == -1 else "➡️  NEUTRO (0)")
            )
            so_str = "🛡️ ATIVO" if sell_only else "❌ inativo"
            console.info(
                f"{hora:<10} {row['close']:>8.0f} {ma60:>10.1f} {diff_pct:>7.3f}% {trend_str:>14} {so_str:>12}"
            )
    ...
    console.info(
        f"   ⏱  Candles com Sell-Only ATIVO (h1_trend=-1) : {sell_only_contagem} / {len(day_data)}"
    )
    console.info(
        f"   📉 Máxima diferença negativa Preço/MA60      : {tendencia_negativa_max:.3f}%"
    )
    ...
    if sell_only_contagem == 0:
        console.info(
            "⚠️  DIAGNÓSTICO: O proxy MA60 M1 ±0.2% foi INSUFICIENTE para detectar a queda do dia 10/03."
        )
        console.info(
            f"   O pior desvio foi {tendencia_negativa_max:.3f}% — abaixo do limiar de -0.200%."
        )
        console.info(
            "   O mercado sofreu uma queda gradual que não atingiu a margem de -0.2% em relação"
        )
        console.info(
            "   à MA de 60min M1. O proxy M1 é muito suave para capturar tendências H1 reais."
        )
        console.info("")
        console.info(
            "💡 SOLUÇÃO PROPOSTA (não implementada ainda - aguardando autorização):"
        )
        console.info(
            "   Usar o resample H1 real no loop do backtest (já disponível na linha 449-458)"
        )
        console.info(
            "   em vez do proxy MA60 M1. O resample H1 real detecta a queda do 10/03 corretamente."
        )
        novomargem = abs(tendencia_negativa_max) * 0.5
        console.info(
            f"   Alternativa: Reduzir margem do proxy de 0.200% para ~{novomargem:.3f}%"
        )
    else:
        console.info(
            f"✅ Sell-Only ativou {sell_only_contagem} candles — verificar por que as compras ainda ocorreram."
        )
