...
async def run_detailed_audit():
    ...
    logging.info("🚀 INICIANDO AUDITORIA DETALHADA SOTA PRO (23/02/2026)")
    ...
    tester = BacktestPro(
        symbol=symbol,
        n_candles=5000, # Pregão de segunda-feira, pegar bastante dado para lookback e o dia
        timeframe="M1",
    ...
    # 3. Executar Simulacao (O BacktestPro cuida do loop)
    await tester.run()
    
    # 4. Processamento de Resultados PRO (Filtrando apenas o dia 23/02)
...
O teste confirma o rigor PRO em {target_date}.
"""

    # Salva o relatório
    report_path = os.path.join(os.path.dirname(__file__), "..", "backend", "audit_pro_2302_report.md")
