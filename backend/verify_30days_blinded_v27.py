...
async def run_blinded_30day_test():
    ...
    if full_df is None or full_df.empty:
        print(
            "❌ Erro: Falha ao carregar dados do MT5. Certifique-se que o MT5 está aberto."
        )
        return
    ...
    if not target_days:
        print(
            "❌ Erro: Não há histórico suficiente para 30 dias. Verifique se o MT5 tem dados de M1 disponíveis."
        )
        return
    ...
    for i, day in enumerate(target_days):
        ...
        if report is None:
            print(
                f"{day.strftime('%d/%m/%Y'):<12} | ERR_SIM    | ---    | ---    | ---      | ❌"
            )
            continue
    ...
    print(
        f"DIAS POSITIVOS:............. {profitable_days} / {len(results)} ({(profitable_days / len(results) * 100) if len(results) > 0 else 0:.1f}%)"
    )
