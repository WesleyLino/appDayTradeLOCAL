...
async def run_deep_audit_11mar():
    ...
    logger.info("-" * 40)
    logger.info(
        f"📈 COMPRAS: {len(compras)} trades | Pnl: R$ {compras[col_pnl].sum() if not compras.empty else 0:.2f}"
    )
    logger.info(
        f"📉 VENDAS:  {len(vendas)} trades | Pnl: R$ {vendas[col_pnl].sum() if not vendas.empty else 0:.2f}"
    )
    logger.info("=" * 40)
