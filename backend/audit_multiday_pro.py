...
async def run_multi_day_audit():
    ...
    if full_data is None or full_data.empty:
        logging.error("❌ Falha ao carregar dados históricos do MT5. Verifique a conexão e símbolos.")
        return
    ...
    for target_date in dates_to_test:
        ...
        if len(day_events) == 0:
            logging.warning(f"⚠️ Sem dados registrados para o dia {target_date.strftime('%d/%m/%Y')}.")
        ...
        if shadow.get('filtered_by_ai', 0) > 15:
            melhorias.append("- **IA Restritiva**: Muitos sinais barrados por incerteza. Considerar leve ajuste no `confidence_threshold` (ex: 0.80).")
        if shadow.get('filtered_by_flux', 0) > 10:
            melhorias.append("- **Fluxo Exigente**: O filtro de agressividade barrou lucros potenciais. O Micro-Flux ajudou, mas pode-se testar `flux_imbalance_threshold` em 1.02.")
        if buy_pnl < -150 and sell_pnl > 150:
            melhorias.append("- **Direcionalidade**: Mercado de forte queda. Bloqueio total de Compras ou redução de lotes em ordens contra-tendência aumentaria a curva.")
        
        if not melhorias:
            melhorias.append("- **Calibragem Ótima**: Defesas e filtros agiram em harmonia para proteger o capital.")
    ...
    report_path = os.path.join(os.path.dirname(__file__), "..", "backend", "audit_multiday_results.md")
