...
def run_single_backtest(params, data_file):
    ...
    try:
        ...
        backtest = BacktestPro(
            data_file=data_file, symbol="WIN", initial_balance=10000.0, **params
        )
...
def optimize():
    ...
    if args.n < len(combinations):
        ...
        logging.info(
            f"🎲 Amostragem aleatória ativa: Reduzido de {len(combinations)} para {args.n} combinações."
        )
    ...
    logging.info(
        f"🧬 Iniciando Otimização AlphaV22 Pro [{args.symbol}] | {len(combinations)} combinações | {args.workers} workers."
    )
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as executor:
        futures = {
            executor.submit(run_single_backtest, params, data_file): params
        ...
        for idx, future in enumerate(concurrent.futures.as_completed(futures)):
            ...
            if "error" not in result:
                logging.info(
                    f"✅ [{args.symbol}] {idx + 1}/{len(combinations)}: PF {result['profit_factor']:.2f} | PnL R${result['net_profit']:.2f} | DD {result['max_dd']:.2%}"
                )
    ...
    champions = sorted(valid_results, key=lambda x: x["profit_factor"], reverse=True)[
        :5
    ]
    ...
    for i, champ in enumerate(champions):
        print(
            f"#{i + 1} | PF: {champ['profit_factor']:.2f} | Lucro: R${champ['net_profit']:.2f} | DD: {champ['max_dd']:.2%}"
        )
