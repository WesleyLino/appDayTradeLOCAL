...
async def run_validated_backtest():
    ...
    params = {
        "rsi_period": 14, "bb_dev": 2.0, "sl_dist": 130.0, "tp_dist": 100.0,
        "vol_spike_mult": 1.2, "use_trailing_stop": False, "be_trigger": 50.0, "be_lock": 0.0,
        "max_daily_trades": 30, "start_time": "09:15", "end_time": "17:15"
    }
    ...
    for i in range(len(df_today)):
        ...
        if bt.position:
            ...
            if exit_type:
                pnl_pts = (exit_price - bt.position['entry_price']) if bt.position['side'] == 'buy' else (bt.position['entry_price'] - exit_price)
                bt.balance += pnl_pts * 0.20
                bt.trades_history.append(pnl_pts)
                logging.info(f"✅ SAÍDA {exit_type} @ {exit_price} | PnL: R$ {(pnl_pts*0.2):.2f}")
                bt.position = None
            elif i - bt.position['index'] > 20: 
                pnl_pts = (row['close'] - bt.position['entry_price']) if bt.position['side'] == 'buy' else (bt.position['entry_price'] - row['close'])
                bt.balance += pnl_pts * 0.20
                bt.trades_history.append(pnl_pts)
                logging.info(f"⏹️ SAÍDA TIME @ {row['close']} | PnL: R$ {(pnl_pts*0.2):.2f}")
                bt.position = None

        # Entrada
        if not bt.position:
            if row.name.time() < datetime.strptime("09:15", "%H:%M").time() or row.name.time() > datetime.strptime("17:15", "%H:%M").time():
                continue
                
            if row['rsi'] < 30 and row['close'] < row['lower_bb'] and row['tick_volume'] > (row['vol_sma'] * params['vol_spike_mult']):
                side = "buy"
            elif row['rsi'] > 70 and row['close'] > row['upper_bb'] and row['tick_volume'] > (row['vol_sma'] * params['vol_spike_mult']):
                side = "sell"
            else: side = None
