...
async def run_analysis():
    ...
    logger.info("=" * 50)
    logger.info(
        f"📊 RESULTADO FINAL (HÍBRIDO): R$ {bt.balance - bt.initial_balance:.2f}"
    )
    ...
    def summarize_side(df_side, side_label):
        ...
        logger.info(
            f"[{side_label}] Lucro Total: R$ {profit_total:.2f} | Trades: {len(df_side)} | Assertividade: {acc:.1f}% | Méd/Trade: R$ {avg_profit:.2f}"
        )
    ...
    results = {
        ...
        "sell_pnl": float(sell_trades["pnl_fin"].sum())
