...
def check_readiness():
    ...
    t_algo_allowed = t_dict.get(
        "trade_expert", False
    )  # No MT5 Python, as vezes é trade_expert

    # Propriedades da Conta
    a_trade_allowed = a_dict.get("trade_allowed", False)
    a_expert_allowed = a_dict.get(
        "trade_expert", False
    )  # Alguns servidores tem trava de EA na conta

    print(f"Trading Permitido no Terminal: {'SIM' if t_trade_allowed else 'NÃO'}")
    print(
        f"AlgoTrading (EA) Permitido no Terminal: {'SIM' if t_algo_allowed else 'NÃO'}"
    )
    print(f"Trading Permitido na Conta: {'SIM' if a_trade_allowed else 'NÃO'}")
    print(
        f"Expert Advisors Permitidos na Conta: {'SIM' if a_expert_allowed else 'NÃO'}"
    )
    ...
    if t_trade_allowed and t_algo_allowed and a_trade_allowed:
        ...
        if not t_algo_allowed:
            print(
                "   -> ATENÇÃO: O botão 'Algo Trading' no topo do MT5 deve estar VERDE."
            )
        if not a_trade_allowed:
            print(
                "   -> ATENÇÃO: A corretora bloqueou o trading nesta conta (Disabled on Server)."
            )
