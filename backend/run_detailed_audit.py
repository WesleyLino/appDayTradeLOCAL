...
async def run_detailed_audit():
    ...
    resumo_tabela += (
        "| Data | PnL Total | Trades | Compra | Venda | Win Rate | Vetos (IA/Bias) |\n"
    )
    resumo_tabela += "| :--- | :--- | :---: | :---: | :---: | :---: | :---: |\n"
    ...
    for target_date in dates_to_test:
        ...
        sliced_data = (
            full_data[mask_until].tail(2000).copy()
        )  # Aumentado para garantir indicadores estáveis
        ...
        ai_instance.uncertainty_threshold_base = (
            0.50  # Permite mais risco para auditoria de potencial
        )

        # Override de parâmetros operacionais do Tester
        tester.opt_params["confidence_threshold"] = (
            0.0  # NEUTRALIZA BUG DE ESTABILIDADE DA VENDA
        )
        ...
        detailed_results += (
            f"- **Defesas**: IA Vetou {vov} sinais, Tendência Vetou {vob} sinais.\n"
        )
