...
def test_ai_core_quantile_dict():
    ...
    score_dict = {"score": 0.92, "q10": 0.80, "q50": 0.91, "q90": 0.98,
                  "forecast_norm": 100.0, "uncertainty_norm": 1.0}
...
def test_rl_shadow_build_state():
    ...
    import numpy as np
    pos = {"ticket": 1, "type": 0, "volume": 1.0,
           "price_open": 130000, "sl": 129850, "tp": 130150,
           "profit": 20.0, "time_open_mins": 15.0}
...
def test_csv_with_cvd():
    ...
    for col_name in ['cvd', 'ofi', 'volume_ratio']:
        assert col_name in df.columns, f"coluna {col_name} ausente no CSV. Colunas: {list(df.columns)}"
...
def test_bot_sniper_lot_scaling():
    ...
    assert "quantile_confidence" in src, "quantile_confidence ausente em bot_sniper_win.py"
    ...
    assert "predict_regime" not in src, "predict_regime ainda presente (deveria ter sido substituido)"
    # Verificar os 3 niveis de lote
    assert '"NORMAL": 1.0' in src or "\"NORMAL\": 1.0" in src, "lote NORMAL=1 nao encontrado"
    assert '"VERY_HIGH": 3.0' in src or "\"VERY_HIGH\": 3.0" in src, "lote VERY_HIGH=3 nao encontrado"
