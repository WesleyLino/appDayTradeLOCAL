...
class BreakevenBacktester(BacktestPro):
    async def load_data(self):
        logging.info(
            f"📥 Coletando 600 candles do DIA ANTERIOR (Loss Day) para {self.symbol}..."
        )
        ...
        rates = await asyncio.to_thread(
            mt5.copy_rates_from_pos, self.symbol, mt5.TIMEFRAME_M1, 0, 600
        )
...
async def run_breakeven_verification():
    ...
    if bt_on.trades:
        for i, trade in enumerate(bt_on.trades):
            print(
                f"Trade {i + 1}: {trade['side'].upper()} | Pts: {trade['pnl_points']} | Motivo: {trade['reason']}"
            )
    else:
