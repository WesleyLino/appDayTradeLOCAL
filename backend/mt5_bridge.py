...
class MT5Bridge:
    ...
    def place_smart_order(self, symbol, order_type, price, volume, sl=0.0, tp=0.0, score=0.0, uncertainty=0.0, comment="SOTA Inteligente"):
        ...
        if use_market:
            logging.info(f"🚀 [SMART ROUTING] Ativando EXECUÇÃO A MERCADO (Score: {score:.1f}, Unc: {uncertainty:.2f})")
            return self.place_market_order(symbol, order_type, volume, sl, tp, comment)
        else:
            logging.info(f"⏳ [SMART ROUTING] Ativando EXECUÇÃO LIMITADA (Score: {score:.1f})")
            return self.place_limit_order(symbol, order_type, price, volume, sl, tp, comment)
