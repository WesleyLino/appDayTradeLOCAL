...
dates = [
    ...
    "2026-03-02", "2026-03-03", "2026-03-04", "2026-03-05", "2026-03-06", "2026-03-09", "2026-03-10", "2026-03-11", "2026-03-12", "2026-03-13"
]

async def run_validation():
    logger.info("🤖 [SOTA-MULTIDAY] Iniciando validação abrangente (Estratégia: Full History Cache + Debug)...")
    ...
    logger.info(f"✅ Histórico carregado. Período BR: {df_all['time_br'].min()} até {df_all['time_br'].max()}")
    ...
    for date_str in dates:
        ...
        df_day = df_all[(df_all['time_br'] >= start_filter) & (df_all['time_br'] <= end_filter)].copy()
        
        if df_day.empty or len(df_day) < 200:
            logger.warning(f"⚠️ Dados insuficientes para {date_str} ({len(df_day)} velas).")
            continue
            
        df_day['time'] = df_day['time_br']
        temp_csv = f"temp_multi_{date_str}.csv"
        df_day[['time', 'open', 'high', 'low', 'close', 'tick_volume', 'spread', 'real_volume']].to_csv(temp_csv, index=False)
        ...
        win_rate = (len([t for t in bt.trades if t['pnl_fin'] > 0]) / len(bt.trades) * 100) if bt.trades else 0
        
        # Log de Justificativa de Veto
        if not bt.trades:
            logger.info(f"🚫 Motivos de veto para {date_str}: {bt.shadow_signals.get('veto_reasons', {})}")
            logger.info(f"🔍 AI Filtered: {bt.shadow_signals.get('filtered_by_ai', 0)} | Flux: {bt.shadow_signals.get('filtered_by_flux', 0)}")
        ...
        logger.info(f"💰 {date_str} | PnL R$ {day_pnl:.2f} | WR: {win_rate:.1f}% | Trades: {len(bt.trades)}")
    ...
    logger.info(f"\n" + "="*60)
    logger.info(f"📊 RELATÓRIO FINAL CONSOLIDADO (SOTA v25)")
    logger.info(f"============================================================")
    for r in results:
        indicator = "✅" if r['pnl'] > 0 else ("💀" if r['pnl'] < 0 else "⚪")
        logger.info(f"{indicator} {r['data']} | Trades: {r['trades']} | PnL: R$ {r['pnl']:8.2f} | WR: {r['win_rate']:5.1f}%")
