...
async def main():
    ...
    scenarios = [
        ...
        ("Sniper Pro (Flux + Limit 3)", {**sniper_base, "daily_trade_limit": 3, "use_flux_filter": True, "flux_imbalance_threshold": 1.5})
    ]
    ...
    # Comparar resultados
    print("\n" + "="*85)
    print(f"{'CENÁRIO PROFISSIONAL':<30} | {'LUCRO':<10} | {'DD':<8} | {'PF':<6} | {'TRADES':<6} | {'WR':<6}")
    print("-" * 85)
    for r in results:
        print(f"{r['name']:<30} | R$ {r['profit']:>8.2f} | {r['drawdown']:>6.2f}% | {r['profit_factor']:>4.2f} | {r['trades']:>6} | {r['win_rate']:>5.1f}%")
    print("="*85)
