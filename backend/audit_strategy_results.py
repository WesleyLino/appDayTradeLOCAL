...
async def run_deep_audit():
    ...
    bt = BacktestPro(
        symbol="WIN$", n_candles=12000, timeframe="M1", initial_balance=3000.0, **params
    )
    ...
    print("\n[+] PONTOS FORTES:")
    print(
        f"- ROI Mensal: {((report['final_balance'] - 3000) / 3000) * 100:.1f}% (Excelente para R$ 3k)"
    )
    ...
    print(
        f"- Relacao Ganho/Perda: {abs(winning_trades['pnl_fin'].mean() / losing_trades['pnl_fin'].mean()):.2f}x (O SOTA Trailing funciona!)"
    )

    print("\n[-] PONTOS NEGATIVOS:")
    print(
        f"- Taxa de Acerto (WR): {report['win_rate']:.1f}% (Pode ser estressante para iniciantes)"
    )
    ...
    print(
        "1. Reduzir Filtro de Fluxo em Regimes de Tendencia (Trend Maestro): Muitos sinais filtrados podem ser lucrativos em tendencia forte."
    )
    print(
        "2. Ajuste de Trailing Trigger: Ativar o trailing um pouco antes (ex: 70 pts) pode garantir lucro em scalps rapidos."
    )
    print(
        "3. Filtro de Horario: Evitar trades apos as 17:00 (Maior volatilidade erratica)."
    )
