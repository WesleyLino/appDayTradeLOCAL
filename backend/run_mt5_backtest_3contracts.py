...
async def main():
    ...
    if df is not None:
        ...
        if len(df_filtered) == 0:
            print(
                f"❌ Nenhum dado encontrado para o dia {target_day} no buffer carregado."
            )
        ...
        if not results or "total_pnl" not in results:
            print(
                "❌ Backtest não gerou resultados válidos (provavelmente sem trades)."
            )
            return
        ...
        if total_pnl > 0:
            print(
                f"O uso de 3 contratos permitiu um ganho de R$ {total_pnl:.2f} em UM DIA."
            )
            print(
                "A assertividade está equilibrada. Os filtros de fluxo estão sendo seletivos."
            )
        else:
