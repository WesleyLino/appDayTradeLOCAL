...
async def main():
    ...
    # Coleta de dados
    rates = mt5.copy_rates_range(symbol, timeframe, date_target, date_target.replace(hour=18, minute=0))
    ...
    h = results["Híbrido"]
    trades = h.trades
    ganho_compra = sum(t['pnl_fin'] for t in trades if t['pnl_fin'] > 0 and t['side'] == 'buy')
    ganho_venda = sum(t['pnl_fin'] for t in trades if t['pnl_fin'] > 0 and t['side'] == 'sell')
    ...
    with open("audit_11mar_final.txt", "w") as f:
        ...
        f.write(f"Liquido:        R$ {(ganho_compra + ganho_venda - prejuizo_total):.2f}\n")
        f.write("-" * 30 + "\n")
        f.write(f"FILTROS:\n")
