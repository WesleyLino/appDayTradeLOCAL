...
async def run_mt5_analysis():
    ...
    if len(trades) > 0:
        ...
        print("\n--- ANÁLISE POR DIREÇÃO ---")
        print(
            f"COMPRAS (BUY):  {len(buy_trades)} trades | PnL: RS {buy_pnl:.2f} | Assertividade: {buy_wr:.2f}%"
        )
        print(
            f"VENDAS (SELL): {len(sell_trades)} trades | PnL: RS {sell_pnl:.2f} | Assertividade: {sell_wr:.2f}%"
        )
    ...
    if shadow.get("filtered_by_flux", 0) > 3:
        print(
            "-> O Filtro de Fluxo esta retendo sinais. Tente 'flux_imbalance_threshold' -1 para desativar."
        )
    if shadow.get("component_fail", {}).get("volume", 0) > 3:
        print(
            "-> Volatilidade alta esta bloqueando entradas. Ajuste 'vol_spike_mult' para 0.8 se desejar mais risco."
        )
    if shadow.get("filtered_by_ai", 0) > 3:
        print(
            "-> A IA está muito restritiva. Verifique o 'confidence_threshold' (atualmente 0.70)."
        )
