...
class Backtester:
    ...
    def load_data(self):
        ...
        if df is None or df.empty:
            logging.error(
                "ERRO: Não foi possível obter dados. Certifique-se que o MT5 está aberto e conectado."
            )
            return None
    ...
    async def run(self):
        ...
        for i in range(seq_len, len(data)):
            ...
            decision = self.ai.calculate_decision(
                mock_obi, mock_sentiment, ai_confidence
            )
            score = decision["score"]
            direction = decision["direction"]

            # 3. Gestão de Risco
            market_cond = self.risk.validate_market_condition(
                regime, current_atr, avg_atr
            )
            risk_ok, _ = self.risk.check_daily_loss(
            ...
            if i + look_ahead < len(data):
                ...
                label = self._calculate_triple_barrier(
                    current_candle["close"], barrier_up, barrier_down, future_slice
                )
                ...
                if (
                    score >= 70
                ):  # Baixei para 70 para ter mais massa de dados de "quase acertos"
            ...
            target_price = current_candle[
                "close"
            ]  # Assumimos execução no fechamento (ou abertura do próximo)

            # Fechamento de Posição (se houver sinal contrário ou stop/gain)
            # Simplificação: Sair se o sinal inverter ou score cair
            if self.position != 0:
                pnl = (
                    (target_price - self.avg_price) * self.position * 0.20
                )  # R$ 0.20 por ponto (WDO/WIN ajustado)
                # Se posição comprada e sinal venda (ou vice versa) -> Zerar
                if (self.position == 1 and direction == "SELL") or (
                    self.position == -1 and direction == "BUY"
                ):
            ...
            if i % 100 == 0:
                print(
                    f"Processado: {i}/{len(data)} | Saldo: {self.balance:.2f}", end="\r"
                )
    ...
    def _calculate_triple_barrier(
        self, entry_price, barrier_up, barrier_down, future_slice
    ):
        """
