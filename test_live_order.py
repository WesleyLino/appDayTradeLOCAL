...
def run_ping_test():
    ...
    result = bridge.place_market_order(
        symbol, mt5.ORDER_TYPE_BUY, volume, sl=0.0, tp=0.0, comment="PING TEST"
    )
    ...
    if result and result.retcode == mt5.TRADE_RETCODE_DONE:
        ...
        if closed:
            logging.info(
                "SUCESSO ABSOLUTO! O Circuito de Compra -> Encerramento de Posições está perfeito."
            )
        else:
            logging.error(
                "Falha ao fechar posição instantaneamente. VERIFIQUE SEU PAINEL DO MT5."
            )
    else:
