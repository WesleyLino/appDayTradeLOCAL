...
async def verify():
    ...
    eff_sent_active = (
        await bot.ai.update_sentiment()
        if (getattr(bot.risk, "enable_news_filter", True))
        else 0.0
    )
    print(
        f"✅ Filtro Ligado: Sentimento Efetivo = {eff_sent_active} (Original: {sentiment_active})"
    )
