...
def run_isolated_backtest(initial_balance=3000.0):
    ...
    for i in range(25, len(df)):
        ...
        if curr_time >= end_time_limit:
            if pos_open:
                pnl = (current_row['close'] - pos_open['price']) * 0.2 * pos_open['qty'] if pos_open['type'] == 'BUY' \
                      else (pos_open['price'] - current_row['close']) * 0.2 * pos_open['qty']
                trades.append({'type': pos_open['type'], 'pnl': pnl, 'reason': 'TIME_LIMIT', 'time': str(current_row['time'])})
                balance += pnl
                pos_open = None
            continue
            
        if pos_open:
            pnl_pts = current_row['close'] - pos_open['price'] if pos_open['type'] == 'BUY' else pos_open['price'] - current_row['close']
            max_pnl_pts = current_row['high'] - pos_open['price'] if pos_open['type'] == 'BUY' else pos_open['price'] - current_row['low']
            min_pnl_pts = current_row['low'] - pos_open['price'] if pos_open['type'] == 'BUY' else pos_open['price'] - current_row['high']
            
            if min_pnl_pts <= sl_pts:
                 pnl = sl_pts * 0.2 * pos_open['qty']
                 trades.append({'type': pos_open['type'], 'pnl': pnl, 'reason': 'STOP_LOSS', 'time': str(current_row['time'])})
                 balance += pnl
                 pos_open = None
                 continue
            ...
            if max_pnl_pts >= tp_pts:
                 ...
                 trades.append({'type': pos_open['type'], 'pnl': pnl, 'reason': 'TAKE_PROFIT', 'time': str(current_row['time'])})
                 balance += pnl
                 pos_open = None
                 continue
        else:
            comprar_cond = current_row['rsi'] < 15 and current_row['real_volume'] > (current_row['vol_sma'] * vol_spike_mult)
            vender_cond = current_row['rsi'] > 85 and current_row['real_volume'] > (current_row['vol_sma'] * vol_spike_mult)
                            
            if comprar_cond or vender_cond:
                side = "buy" if comprar_cond else "sell"
                regime = ai.detect_regime(current_row['atr'], 1.0)
                market_val = risk.validate_market_condition("WIN$", regime, current_row['atr'], df['atr'].mean())
                ...
                if market_val["allowed"]:
                    ...
                    opportunities_missed.append({'time': str(current_row['time']), 'side': side, 'reason': market_val['reason']})
