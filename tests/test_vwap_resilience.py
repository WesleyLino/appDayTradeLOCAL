...
class MockBridge:
    ...
    def get_settlement_price(self, s):
        return 130000.0
    ...
    def get_htf_bias(self, s):
        return "LONG"

    def get_real_cvd_ticks(self, s):
        return 0.0

    def get_daily_volume_and_liquidity(self, s):
        return {}
...
class TestGarantiaAbsoluta(unittest.TestCase):
    ...
    def test_sticky_logic(self):
        ...
        if vwap_fail > 0:
            self.worker._last_vwaps["WIN$"] = vwap_fail
        # Aplica lógica Sticky Settle
        if settle_fail > 0:
            self.worker._last_settlements["WIN$"] = settle_fail

        self.assertEqual(
            self.worker._last_vwaps["WIN$"], 130000.0, "VWAP deveria ser Sticky"
        )
        self.assertEqual(
            self.worker._last_settlements["WIN$"],
            129500.0,
            "Settlement deveria ser Sticky",
        )
