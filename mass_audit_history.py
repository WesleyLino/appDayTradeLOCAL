...
class MassAuditMotor:
    ...
    async def run_audit(self):
        logging.info(f"🔍 Iniciando Auditoria Global para {self.symbol} ({self.days_back} dias)")
    ...
    async def audit_day(self, day_date):
        ...
        for cfg in configs:
            try:
                ...
                data = self.bridge.get_market_data_range(self.symbol, mt5.TIMEFRAME_M1, date_from, date_to)
                ...
                win_rate = (len(df_trades[df_trades['pnl_fin'] > 0]) / len(df_trades) * 100) if not df_trades.empty else 0.0
    ...
    def generate_summary_report(self):
        ...
        expected_cols = [
            "LEGACY_V22_pnl", "LEGACY_V22_trades", "LEGACY_V22_winrate",
            "SOTA_V2_AI_pnl", "SOTA_V2_AI_trades", "SOTA_V2_AI_winrate"
        ]
