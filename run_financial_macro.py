...
DATAS_TESTE = [
    (2026, 2, 19), (2026, 2, 20), (2026, 2, 23), (2026, 2, 24), (2026, 2, 25), (2026, 2, 26), (2026, 2, 27),
    (2026, 3, 2),  (2026, 3, 3),  (2026, 3, 4),  (2026, 3, 5),  (2026, 3, 6),  (2026, 3, 9),  (2026, 3, 10)
]
...
def simular_dia(ano, mes, dia, params):
    ...
    if df.empty: return {"compras": [], "vendas": [], "perdidas": [], "valido": False}
    ...
    for i in range(25, len(df)):
        ...
        if current['high'] - current['low'] > 150 and not pos_open:
            oportunidades_perdidas.append({'amplitude': current['high'] - current['low']})
    ...
    return {"compras": compras_realizadas, "vendas": vendas_realizadas, "perdidas": oportunidades_perdidas, "valido": True}
...
def run_all_days():
    ...
    for a, m, d in DATAS_TESTE:
        ...
        print(f"► [{dia_str}] PnL: R$ {pnl_dia:>7.2f} | Compras: {len(c)} | Vendas: {len(v)} | Defesas(BE): {qt_be} | Losses: {qt_sl} | Big Pernas Iguais: {len(res['perdidas'])}")
    ...
    print(f"  TOTAL DE ENTRADAS       : {total_compras + total_vendas} trades (C:{total_compras} V:{total_vendas})")
    print(f"  BLINDAGENS P/ BREAK-EVEN: {total_be} trades (Eram perdas que fecharam em R$ 0,00)")
