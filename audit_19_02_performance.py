    ...
    for cfg in configs:
        ...
        if len(trades) > 0:
            ...
            if not cfg["use_ai"]:
                ...
                for t in trades:
                    print(
                        f"  [{t['entry_time'].strftime('%H:%M')}] {t['side'].upper()} @ {t['entry']:.2f} -> {t['exit']:.2f} | PnL: RS {t['pnl_fin']:.2f} ({t['reason']})"
                    )

    print(f"\n{'=' * 60}")
    print("RESUMO DA AUDITORIA 19/02:")
    print(
        "Análise do impacto do Refinamento de Assertividade em dia de alta volatilidade."
    )
    print(f"{'=' * 60}\n")
