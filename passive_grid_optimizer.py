...
def run_grid_search_optimisation():
    ...
    total_combs = (
        len(grid_rsi_periods)
        * len(grid_obi_thresholds)
        * len(grid_rsi_buy_levels)
        * len(grid_rsi_sell_levels)
    )
                ...
                for r_sell in grid_rsi_sell_levels:
                    ...
                    for i in range(len(df_opt) - 10):
                        ...
                        if row["rsi"] < r_buy and flux_ok:
                            mfe = (
                                df_opt.iloc[i + 1 : i + 11]["high"].max() - row["close"]
                            )
                            mae = (
                                row["close"] - df_opt.iloc[i + 1 : i + 11]["low"].min()
                            )
                    ...
                    if buy_wins + buy_losses > 0:
                        b_score = (buy_wins / (buy_wins + buy_losses)) * (
                            buy_wins - (buy_losses * 0.5)
                        )
                    ...
                    if sell_wins + sell_losses > 0:
                        s_score = (sell_wins / (sell_wins + sell_losses)) * (
                            sell_wins - (sell_losses * 0.5)
                        )
    ...
    print("\n[COMPRA / LONG PATTERN]")
    print(
        f"Melhor Configuração: RSI Period: {best_buy['params'].get('RSI_P')} | OBI Vol: {best_buy['params'].get('OBI')} | Límite RSI: {best_buy['params'].get('RSI_B')}"
    )
    print(
        f"Performance: {best_buy.get('wins')} Ganhos (>70pts) vs {best_buy.get('losses')} Perdas Letais (WinRate: {(best_buy.get('wins', 0) / (max(1, best_buy.get('wins', 0) + best_buy.get('losses', 0)))) * 100:.1f}%)"
    )
    if best_buy.get("wins", 0) <= best_buy.get("losses", 0):
        print(
            "[!] ALERTA: Mesmo otimizado, o viés do dia para compra foi matematicamente tóxico."
        )

    print("\n[VENDA / SHORT PATTERN]")
    print(
        f"Melhor Configuração: RSI Period: {best_sell['params'].get('RSI_P')} | OBI Vol: {best_sell['params'].get('OBI')} | Límite RSI: {best_sell['params'].get('RSI_S')}"
    )
    print(
        f"Performance: {best_sell.get('wins')} Ganhos (>70pts) vs {best_sell.get('losses')} Perdas Letais (WinRate: {(best_sell.get('wins', 0) / (max(1, best_sell.get('wins', 0) + best_sell.get('losses', 0)))) * 100:.1f}%)"
    )
