...
async def rodar_backtest(label_modo, modo_params, df):
    ...
    forced = sum(
        1 for t in trades if t.get("reason", "") in ("ALPHA_DECAY", "FORCE_CLOSE")
    )
    ...
    print(f"{'─' * 60}")
    print(
        f"  PnL Total    : R${pnl:+.2f}  (C: R${pnl_buy:+.2f} | V: R${pnl_sell:+.2f})"
    )
    ...
    if trades:
        ...
        for i, t in enumerate(trades, 1):
            ...
            print(
                f"    [{i:02d}] {lado} {hora_e}→{hora_s} | {entry:.0f}→{exit_p:.0f} ({pnl_p:+.0f}pts) | {reason:12} | {lots}x | R${pnl_t:+.2f} | [{modo_e}]"
            )
...
async def main():
    try:
        ...
        print(
            f"   Mín: {df['low'].min():.0f} | Máx: {df['high'].max():.0f} | Δ: {df['high'].max() - df['low'].min():.0f}pts"
        )
        print(
            f"   Variação (%): {((df['close'].iloc[-1] - df['close'].iloc[0]) / df['close'].iloc[0] * 100):+.2f}%"
        )
        ...
        print(f"{'=' * 62}")
        print(
            f"  {'Modo':<30} {'PnL':>9} {'T':>4} {'WR':>6} {'C':>3} {'V':>3} {'TP':>3} {'SL':>3}"
        )
        print(f"  {'─' * 60}")
        for r in resultados:
            destaque = "✅" if r["pnl"] == max(x["pnl"] for x in resultados) else "  "
            print(
                f"  {destaque} {r['modo']:<28} R${r['pnl']:>+.2f} {r['trades']:>4} {r['wr']:>5.1f}% {r['buys']:>3} {r['sells']:>3} {r['tps']:>3} {r['sls']:>3}"
            )
        ...
        print(
            f"     PnL: R${melhor['pnl']:+.2f} | Trades: {melhor['trades']} | WR: {melhor['wr']:.1f}%"
        )
        print(
            f"     Compras: R${melhor['pnl_buy']:+.2f} | Vendas: R${melhor['pnl_sell']:+.2f}"
        )
