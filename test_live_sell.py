...
def run_ping_test_sell():
    ...
    result = bridge.place_market_order(symbol, mt5.ORDER_TYPE_SELL, volume, sl=0.0, tp=0.0, comment="PING TEST SELL")
    
    if result and result.retcode == mt5.TRADE_RETCODE_DONE:
        ticket = result.deal
        logging.info(f"SUCESSO! Ordem de venda executada no MT5. Ticket do Deal: {ticket}")
        ...
        if closed:
            logging.info("SUCESSO ABSOLUTO! O Circuito de Venda -> Encerramento de Posições está perfeito.")
        else:
            logging.error("Falha ao fechar posição de venda instantaneamente. VERIFIQUE SEU PAINEL DO MT5.")
    else:
        logging.error(f"Falha ao executar ordem de venda inicial. Retcode: {result.retcode if result else 'N/A'}")
