...
async def run_audit_19_02():
    bridge = MT5Bridge()

    # Data de Interesse: 19/02/2026
    ...
    print("\n" + "=" * 70)
    print("📊 AUDITORIA DE PERFORMANCE SOTA v3.1: 19/02/2026 | CAPITAL: R$ 3.000,00")
    print("=" * 70)
    ...
    if data is None or data.empty:
        print("❌ ERRO: Não há dados para 19/02 no arquivo MASTER.")
        return

    print(f"✅ {len(data)} velas carregadas para o pregão de 19/02.")
    ...
    print("\n" + "=" * 70)
    print("🏆 RESULTADO COMPARATIVO: 19/02 (WIN$)")
    print("-" * 70)
    ...
    print(
        f"PNL SOTA v3.1:  R$ {pnl_sota:>10.2f} ({trades_sota} trades, WR: {wr_sota:.1f}%)"
    )
    print(
        f"PNL LEGACY V22: R$ {pnl_legacy:>10.2f} ({trades_legacy} trades, WR: {wr_legacy:.1f}%)"
    )
