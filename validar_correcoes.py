...
async def rodar_dia(label, arquivo):
    ...
    sys.stdout.write(
        f"         Vetos IA -> Compra: {buy_vetos_ia} | Venda: {sell_vetos_ia}\n"
    )
    sys.stdout.write(f"         Top Vetos: {vetos_str}\n")
    sys.stdout.write(
        f"         TPs: {sum(1 for t in trades if t.get('exit_type', '') in ('TP_DECAY', 'TP'))} | SLs: {sum(1 for t in trades if t.get('exit_type', '') in ('SL',))}\n\n"
    )
    sys.stdout.flush()
    return pnl
