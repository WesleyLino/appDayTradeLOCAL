...
async def diagnose():
    ...
    if not potential.empty:
        ...
        print(
            potential[
                ["close", "rsi", "upper_bb", "lower_bb", "tick_volume", "vol_sma"]
            ].head(20)
        )
    ...
    for idx, row in potential.head(10).iterrows():
        ...
        print(
            f"Horário: {idx} | RSI OK: {rsi_ok} ({row['rsi']:.1f}) | BB OK: {bb_ok} | Vol OK: {vol_ok} (V: {row['tick_volume']} vs SMA: {row['vol_sma']:.1f})"
        )
