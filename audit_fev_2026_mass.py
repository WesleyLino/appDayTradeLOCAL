...
async def run_mass_audit():
    ...
    print("\n" + "="*80)
    print(f"--- START MASS AUDIT: SOTA PRO (REFINADA) | CAPITAL: R$ {initial_balance:,.2f} | SYMBOL: {symbol}")
    print("="*80)
    ...
    for audit_date in dates:
        ...
        if results:
            ...
            print(f"[OK] DIA {date_str}: PNL R$ {results['total_pnl']:.2f} | Trades: {len(results['trades'])} | Shadows: {len(results['shadow_signals'])}")
    ...
    if all_results:
        ...
        print(f"OPORTUNIDADES DE COMPRA    : {summary_df['shadow_buy_count'].sum():>12}")
        print(f"OPORTUNIDADES DE VENDA     : {summary_df['shadow_sell_count'].sum():>12}")
        print("-" * 80)
