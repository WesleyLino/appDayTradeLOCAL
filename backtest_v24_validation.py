...
async def run_v24_validation():
    ...
    try:
        ...
        params_golden = risk.get_order_params(
            "WIN$", mt5.ORDER_TYPE_BUY_LIMIT, 130000, 2, 
            current_atr=150, regime=1, current_time=t_golden
        )
        ...
        tr_sell, lock_sell, step_sell = risk.get_dynamic_trailing_params(150, side="sell")
        
        logger.info(f"🛡️ Trailing Compra: Step={step_buy} | Venda: Step={step_sell}")
        if step_sell < step_buy:
            logger.info("✅ SUCESSO: Trailing Assimétrico (Venda mais agressiva) configurado.")
        else:
