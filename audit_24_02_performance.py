...
async def detailed_audit():
    ...
    print(f"\n{'='*60}")
    print("AUDITORIA DETALHADA: POTENCIAL DE GANHO WIN$ (24/02/2026)")
    print(f"{'='*60}")
    
    symbol = "WIN$"
    capital = 3000.0
    target_date = datetime(2026, 2, 24).date()

    configs = [
        {"name": "V22 GOLDEN (LOCKED)", "use_ai": True, "conf_thresh": 0.70, "flux_thresh": 1.2},
        {"name": "BALANCED (TREINED)", "use_ai": True, "conf_thresh": 0.65, "flux_thresh": 1.1},
        {"name": "AGGRESSIVE (POTENTIAL)", "use_ai": True, "conf_thresh": 0.60, "flux_thresh": -1.0}, # Desativa Fluxo
        {"name": "RAW (NO AI)", "use_ai": False, "conf_thresh": 0.0, "flux_thresh": -1.0} # Sem IA, Sem Fluxo
    ]
    ...
    for cfg in configs:
        ...
        tester = BacktestPro(
            symbol=symbol,
            n_candles=3000, # Aumentado para pegar o dia anterior completo
            timeframe="M1",
    ...
    print("1. Análise comparativa com cenário Real-Data de ontem.")
    print("2. Verificação de reentradas e preservação de capital.")
    print(f"{'='*60}\n")
