...
async def run_comparison():
    ...
    symbol = "WIN$N"
    dias = [
        datetime(2026, 2, 19), datetime(2026, 2, 20),
        datetime(2026, 2, 23), datetime(2026, 2, 24),
        datetime(2026, 2, 25), datetime(2026, 2, 26),
        datetime(2026, 2, 27), datetime(2026, 3, 2)
    ]
    ...
    for dia in dias:
        ...
        data = bridge.get_market_data_range(symbol, mt5.TIMEFRAME_M1, dia.replace(hour=7, minute=0), dia.replace(hour=17, minute=45))
        ...
        print(f"  [{date_str}] Cands: {cands:>2} | Trades (93%): {len(res_93.get('trades',[]))} [PnL: R${pnl_93:>7.2f}] | Trades (82%): {len(res_82.get('trades',[]))} [PnL: R${pnl_82:>7.2f}]")
