...
async def analise_dia():
    ...
    # Filtrar dia 06/03
    day_df = df[(df['time'] >= "2026-03-06 09:00:00") & (df['time'] <= "2026-03-06 18:00:00")].copy()
    ...
    print(f"Assertividade COMPRA: {len(win_buys)}/{len(buys)} ({len(win_buys)/len(buys)*100:.1f}%)" if buys else "Nenhuma COMPRA")
    print(f"Assertividade VENDA: {len(win_sells)}/{len(sells)} ({len(win_sells)/len(sells)*100:.1f}%)" if sells else "Nenhuma VENDA")
