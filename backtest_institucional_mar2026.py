...
async def run():
    ...
    _arquivo_saida.write(
        "  SIMULAÇÃO INSTITUCIONAL SOTA (v50.0 MASTER) - MINI ÍNDICE (WIN)\n"
    )
    ...
    for dia in DIAS_ALVO:
        ...
        data = bridge.get_market_data_range(
            SYMBOL, mt5.TIMEFRAME_M1, date_from_buffer, date_to
        )
        ...
        _arquivo_saida.write(
            f"📊 [{date_str}] Dados: {len(data)} candles carregados.\n"
        )
        ...
        _arquivo_saida.write(
            f"  [{date_str}] Trades: {len(day_trades):>2} | PnL: {fmt_reais(day_pnl):>12} | WR: {win_rate:>5.1f}% | Cand: {cand_dia:>2} | Vetos: {vetos_ia}\n"
        )
    ...
    _arquivo_saida.write("-" * 85 + "\n")
    _arquivo_saida.write(
        f"  PATRIMÔNIO FINAL       : {fmt_reais(INITIAL_BAL + total_pnl)}\n"
    )
    ...
    if not trades_df.empty:
        ...
        _arquivo_saida.write(
            f"  ├── PnL COMPRAS: {fmt_reais(compras['pnl_fin'].sum())}\n"
        )
        _arquivo_saida.write(
            f"  └── PnL VENDAS : {fmt_reais(vendas['pnl_fin'].sum())}\n"
        )
