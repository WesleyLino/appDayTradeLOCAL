...
async def run_diagnostics():
    ...
    if not bridge.connect():
        print(
            "❌ ERRO: Falha ao conectar ao MetaTrader 5. Verifique se o Terminal está aberto."
        )
        return
    ...
    try:
        ...
        print(
            f"💰 Equity Atual: {acc_info.equity} | Alavancagem: 1:{acc_info.leverage}"
        )
    ...
    if multi_data is not None and not multi_data.empty:
        ...
        print(
            "❌ ERRO: Falha na sincronização de dados. Verifique a liquidez dos ativos."
        )

    # 3. Model Loading & Inference (ONNX + DML)
    print("\n🧠 [3/5] TESTANDO CÉREBRO SOTA (ONNX + DIRECTML)...")
    weights_path = (
        "backend/patchtst_weights_sota.pth"  # O engine converte para .onnx internamente
    )
    ...
    if multi_data is not None:
        try:
            ...
            if isinstance(preds, dict):
                print(
                    f"✅ Inferência Concluída: Forecast={preds['forecast_norm']:.4f} | Incerteza={preds['uncertainty_norm']:.4f}"
                )
    ...
    if book["bids"] and book["asks"]:
        print(
            f"✅ Book de Ofertas Ativo: {len(book['bids'])} Bids | {len(book['asks'])} Asks"
        )
