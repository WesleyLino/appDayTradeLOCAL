...
def simular_dia(data_foco, label_dia):
    ...
    for i in range(25, len(df)):
        ...
        if c_time < time(9, 15) or c_time >= time(17, 15):
            if pos_open:
                pnl = current['close'] - pos_open['price'] if pos_open['type'] == 'BUY' else pos_open['price'] - current['close']
                pos_open['pnl'] = pnl
        ...
        if pos_open:
            ...
            if vol_condition:
                if current['rsi'] <= 30:
                    pos_open = {'type': 'BUY', 'price': current['close'], 'time': str(c_time)}
                elif current['rsi'] >= 70:
                    pos_open = {'type': 'SELL', 'price': current['close'], 'time': str(c_time)}
