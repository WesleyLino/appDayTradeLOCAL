...
def generate_report():
    ...
    for c in compras:
        ...
        print(
            f"  - {c['time']} | Resultado: {c['reason']:<12} | RunUp Max: {c.get('max_runup', 0):>5} pts | PnL: R$ {lucro_rs:>6.2f}"
        )
    ...
    for v in vendas:
        ...
        print(
            f"  - {v['time']} | Resultado: {v['reason']:<12} | RunUp Max: {v.get('max_runup', 0):>5} pts | PnL: R$ {lucro_rs:>6.2f}"
        )
    ...
    print(
        f"Identificamos {len(movimentos_gigantes)} candles que se moveram mais de 150 pts."
    )
    ...
    for p in movimentos_gigantes:
        ...
        print(
            f"  - {p['time'][:5]} | Direção: {p['direcao']:<5} | Amplitude: {p['amplitude']} pts | Atr na hora: {p['atr_momento']:.1f}"
        )
    ...
    print(
        "2. A principal trava foi de volatilidade. No horario das 09h00, o filtro ATR atua melhor em 40 do que em 50."
    )
    print("=======================================================\n")
