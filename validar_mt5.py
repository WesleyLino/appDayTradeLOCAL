...
async def rodar_dia_mt5(label, data_str):
    try:
        ...
        vetos_str = (
            " | ".join(f"{k}={v}" for k, v in vetos_top) if vetos_top else "nenhum"
        )

        h1_byp = sum(1 for t in trades if "H1 BYPASS" in str(t.get("reason", "")))

        sys.stdout.write(
            f"[{label}] PnL: R${pnl:+.2f} | Trades: {total} (C:{buys}/V:{sells}) | WR: {wr:.1f}%\n"
        )
        sys.stdout.write(f"         Top Vetos: {vetos_str}\n")
        sys.stdout.write(
            f"         TPs: {sum(1 for t in trades if t.get('exit_type', '') in ('TP_DECAY', 'TP'))} | SLs: {sum(1 for t in trades if t.get('exit_type', '') in ('SL',))} | H1_Bypass: {h1_byp}\n\n"
        )
        sys.stdout.flush()
        return pnl
