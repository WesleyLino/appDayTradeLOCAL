...
async def rigorous_sync_test():
    ...
    try:
        ...
        async with websockets.connect(WS_URI, additional_headers={"Origin": ORIGIN}) as websocket:
            print("Conectado ao WebSocket de Auditoria.")
            print("-" * 100)
            print(f"{'SÍMBOLO':<8} | {'HORÁRIO MT5':<15} | {'HORÁRIO APP (BR)':<18} | {'MT5':<8} | {'APP':<8} | {'DELAY'}")
            print("-" * 100)
            ...
            for i in range(15):
                try:
                    ...
                    app_time = datetime.fromtimestamp(app_ts).strftime('%H:%M:%S.%f')[:-3] if app_ts else "N/A"
                    ...
                    if mt5_tick:
                        ...
                        mt5_time_raw = datetime.fromtimestamp(mt5_tick.time) - timedelta(hours=3)
                        ...
                        print(f"{symbol:<8} | {mt5_time_str:<15} | {app_time:<18} | {mt5_price:<8.1f} | {color}{app_price:<8.1f}{reset} | {latency:.1f}ms")
                    else:
                        print(f"{symbol:<8} | {'N/A':<15} | {app_time:<18} | {'N/A':<8} | {app_price:<8.1f} | Sincronia de Tick pendente...")
