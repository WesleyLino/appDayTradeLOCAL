...
async def run_audit():
    ...
    # Formato: (Ano, Mês, Dia)
    target_dates = [
        (2026, 2, 19), (2026, 2, 20), (2026, 2, 23), (2026, 2, 24),
        (2026, 2, 25), (2026, 2, 26), (2026, 2, 27), (2026, 2, 28),
        (2026, 3, 2), (2026, 3, 3)
    ]
    
    print("=== AUDITORIA DE 10 DIAS (M1) - R$ 3000 INICIAIS | CALIBRAGEM v52.4 ===")
    print(f"{'Data':<12} | {'Win Rate':<8} | {'Trades':<6} | {'(C/V)':<7} | {'PnL BRL':<10} | {'Missed':<6}")
    print("-" * 65)
    ...
    for year, month, day in target_dates:
        ...
        rates = bridge.mt5.copy_rates_range("WINJ26", bridge.mt5.TIMEFRAME_M1, start_dt, end_dt)
        
        if rates is None or len(rates) == 0:
            print(f"{day:02d}/{month:02d}/{year}   | SEM DADOS (Final de Semana/Feriado/Missing)")
            continue
        ...
        print(f"{day:02d}/{month:02d}/{year}   | {wr:>5.1f}%   | {trades_count:<6} | {cv_str:<7} | {c_pnl:<10} | {missed:<6}")

    print("-" * 65)
    global_wr = (total_wins / total_trades * 100) if total_trades > 0 else 0.0
    print(f"RESULTADO FINAL: {total_trades} trades | WinRate: {global_wr:.1f}% | PnL Total: R$ {total_pnl:+.2f}")
