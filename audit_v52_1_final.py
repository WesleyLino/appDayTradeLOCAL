...
wp = bp["params"]
print(
    f"  daily_trade_limit   : {wp.get('daily_trade_limit', 'AUSENTE')}  (esperado: 999)"
)
print(
    f"  confidence_threshold: {wp.get('confidence_threshold', 'AUSENTE')}   (esperado: 0.75)"
)
print(f"  be_trigger          : {wp.get('be_trigger', 'AUSENTE')}    (esperado: 40.0)")
print(
    f"  partial_profit_pts  : {wp.get('partial_profit_points', 'AUSENTE')}    (esperado: 45.0)"
)
...
erros = []
if r.daily_trade_limit != 999:
    erros.append("ERRO: RiskManager.daily_trade_limit != 999")
if r.be_trigger != 40.0:
    erros.append("ERRO: RiskManager.be_trigger != 40.0")
if r.partial_profit_points != 45.0:
    erros.append("ERRO: RiskManager.partial_profit_points != 45.0")
if ai.buy_threshold != 75.0:
    erros.append("ERRO: AICore.buy_threshold != 75.0")
if ai.sell_threshold != 25.0:
    erros.append("ERRO: AICore.sell_threshold != 25.0")
if sp["daily_trade_limit"] != 999:
    erros.append("ERRO: JSON.daily_trade_limit != 999")
if sp["confidence_threshold"] != 0.75:
    erros.append("ERRO: JSON.confidence_threshold != 0.75")
if sp["be_trigger"] != 40.0:
    erros.append("ERRO: JSON.be_trigger != 40.0")
if wp.get("daily_trade_limit") != 999:
    erros.append("ERRO: best_params_WIN.daily_trade_limit != 999")
if wp.get("confidence_threshold") != 0.75:
    erros.append("ERRO: best_params_WIN.confidence_threshold != 0.75")
