    ...
    def forward(self, x, mode):
        if mode == "norm":
            ...
            self.sigma = torch.sqrt(
                torch.var(x, dim=1, keepdim=True) + self.eps
            ).detach()
...
class UltraStablePatchTST(nn.Module):
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [ManualTransformerBlock(d_model, n_heads) for _ in range(n_layers)]
        )
...
def run():
    ...
    model = UltraStablePatchTST(
        c_in=len(z_df.columns),
        context_window=60,
        target_window=5,
        patch_len=8,
        stride=4,
    ...
    for epoch in range(5):
        ...
        for i, (x, y) in enumerate(dl):
            ...
            if i % 10 == 0:
                logging.info(
                    f"--- HEARTBEAT | E{epoch + 1} B{i}/{len(dl)} | Loss: {loss.item():.4f} ---"
                )
