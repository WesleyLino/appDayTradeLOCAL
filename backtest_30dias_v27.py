...
async def run():
    ...
    print(
        f"  PARAMS   : flux={locked_params.get('flux_imbalance_threshold', '?')}, "
    ...
    for dia in DIAS_ALVO:
        bt = BacktestPro(symbol=SYMBOL, initial_balance=INITIAL_BAL, **locked_params)
        ...
        for i in range(20, len(data) - 1):
            ...
            try:
                ...
                vol_ok = float(row.get("tick_volume", 1)) > float(
                    seg["tick_volume"].rolling(20).mean().iloc[-1]
                ) * float(locked_params.get("vol_spike_mult", 1.0))
                ...
                if side:
                    ...
                    flux_threshold = float(
                        locked_params.get("flux_imbalance_threshold", 1.05)
                    )
                    bid_proxy = float(seg["tick_volume"].iloc[-5:].mean())
                    ask_proxy = (
                        float(seg["tick_volume"].iloc[-10:-5].mean())
                        if len(seg) >= 10
                        else bid_proxy
                    )
                    flux_ratio = (
                        (bid_proxy / (ask_proxy + 1e-9))
                        if side == "buy"
                        else (ask_proxy / (bid_proxy + 1e-9))
                    )
        ...
        equity_arr = (
            bt.equity_curve
            if hasattr(bt, "equity_curve") and bt.equity_curve
            else [INITIAL_BAL]
        )
        dd_pct = (
            ((max(equity_arr) - min(equity_arr)) / max(equity_arr) * 100)
            if len(equity_arr) > 1
            else 0.0
        )
        ...
        print(
            f"  ├── PNL Total          :  {fmt_reais(pnl_dia):>15}   {pct(pnl_dia, INITIAL_BAL)}"
        )
        ...
        print(
            f"  Qtd: {n_buy}  |  Ganhos: {buy_wins}  |  Perdas: {buy_losses}  |  PNL: {fmt_reais(pnl_buy)}"
        )
        if buys:
            print(
                f"  {'Entrada':<22} {'Saída':<22} {'Preço E':>9} {'Preço S':>9} {'Pts':>7} {'PNL':>13}  Motivo"
            )
            print(f"  {'-' * 95}")
            for t in buys:
                pts = t.get("pnl_pts", 0)
                ...
                print(
                    f"  {icon} {ei}   {eo}  {t.get('entry_price', 0):>9.0f}  {t.get('exit_price', 0):>9.0f}  {pts:>+7.0f}  {fmt_reais(t['pnl_fin']):>12}  {t.get('reason', '')}"
                )
        ...
        print(
            f"  Qtd: {n_sell}  |  Ganhos: {sell_wins}  |  Perdas: {sell_losses}  |  PNL: {fmt_reais(pnl_sell)}"
        )
        if sells:
            print(
                f"  {'Entrada':<22} {'Saída':<22} {'Preço E':>9} {'Preço S':>9} {'Pts':>7} {'PNL':>13}  Motivo"
        ...
        print(
            f"  Aproveitamento            : {n_trades}/{candidates} ({n_trades / candidates * 100:.1f}%)"
            if candidates > 0
            else "  Aproveitamento: N/A"
        )
        print(f"  {SEP_SIMPL}")
    ...
    print(
        f"  ├── PNL Consolidado       : {fmt_reais(total_pnl):>20}   {pct(total_pnl, INITIAL_BAL)}"
    )
    ...
    if resumo_dias:
        ...
        print(
            f"  ├── Melhor Dia            : {melhor['data']} → {fmt_reais(melhor['pnl_total'])}"
        )
        print(
            f"  └── Pior Dia              : {pior['data']} → {fmt_reais(pior['pnl_total'])}"
        )
        print()
        print("  OPERAÇÕES")
        ...
        print(
            f"  ├── Win Rate Médio        : {sum(wrs) / len(wrs):.1f}%"
            if wrs
            else "  ├── Win Rate Médio: N/A"
        )
        print(
            f"  ├── Profit Factor Médio   : {sum(pfs) / len(pfs):.2f}"
            if pfs
            else "  ├── Profit Factor: N/A"
        )
        print(
            f"  └── Max Drawdown (pior dia): {max(dds):.2f}%"
            if dds
        ...
        print(
            f"  └── Taxa aproveitamento   : {total_trades}/{total_candidates} ({total_trades / total_candidates * 100:.1f}%)"
            if total_candidates > 0
            else "  └── Taxa aproveitamento: N/A"
        )
    ...
    print()
    print(
        f"  {'DATA':<14} {'TRADES':>6} {'BUY':>5} {'SELL':>5} {'PNL COMPRA':>12} {'PNL VENDA':>12} {'PNL TOTAL':>12} {'WR%':>6} {'PF':>6}"
    )
    print(f"  {'-' * 88}")
    for d in resumo_dias:
        pf_str = f"{d['pf']:.2f}" if d["pf"] != float("inf") else "∞"
        print(
            f"  {d['emoji']} {d['data']:<12} {d['trades']:>6} {d['buy']:>5} {d['sell']:>5} "
    ...
    if resumo_dias:
        print(f"  {'─' * 88}")
        print(
            f"  {'TOTAL':<14} {total_trades:>6} {total_buy:>5} {total_sell:>5} "
    ...
    if resumo_dias:
        ...
        if avg_wr > 70:
            print(
                f"  [POSITIVO] Win Rate médio {avg_wr:.1f}% acima de 70%: sistema com alta assertividade."
            )
        if avg_pf > 2.0:
            print(
                f"  [POSITIVO] Profit Factor médio {avg_pf:.2f}: retorno muito consistente."
            )
        if total_blocked_flux > 0:
            flux_pct = (
                total_blocked_flux / total_candidates * 100
                if total_candidates > 0
                else 0
            )
            print(
                f"  [ANALISE] {total_blocked_flux} sinais bloqueados pelo flux ({flux_pct:.1f}% dos candidatos)."
            )
            if flux_pct > 5:
                print(
                    "           → Avaliar redução de flux_threshold para 0.95 (requer validação)."
                )
        dias_neg = [d for d in resumo_dias if d["pnl_total"] < 0]
        if dias_neg:
            print(
                f"  [ALERTA] {len(dias_neg)} dia(s) negativo(s): {', '.join(d['data'] for d in dias_neg)}"
            )
            print(
                "           → Analisar padrão de mercado nesses dias para regras de veto."
            )
        else:
            print(
                f"  [EXCELENTE] Todos os {len(resumo_dias)} dias analisados foram positivos."
            )
